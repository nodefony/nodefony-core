...
(`client/realtime/RealtimeClient.ts:1056`) — plafonné à 30 secondes par défaut. La date de la
...
| `RealtimeClient.stream()`             | `client/realtime/RealtimeClient.ts:735` | Réponse en morceaux (génération jeton par jeton), résolue à la fin |
...
(`client/realtime/RealtimeClient.ts:783`) fait du navigateur un **appelé** : le serveur peut lui
...
(`client/realtime/RealtimeClient.ts:386`) pour une réaction précise (griser le contrôle concerné). Le
...
- **Le journal de protocole est différé.** Chaque trame est poussée dans un anneau borné à 300
  ...
  faits qu'à la **lecture** — `recordFrame()` (`client/realtime/RealtimeClient.ts:1113`). Un
...
| `RealtimeIdentity` introuvable à l'import                       | Réexporté par le module, **pas** par le barrel client (`client/realtime/RealtimeClient.ts:82`)                | Laisser l'inférence faire (`socket.identity`), ou déclarer son propre type                   |
| Le canal est silencieux, aucun message                          | `on()` installé sans `subscribe()` — le serveur ne pousse pas                                                 | Appeler les deux (`client/realtime/RealtimeClient.ts:430`)                                   |
...
| Les trames envoyées juste après la connexion sont perdues       | `send()` abandonne la trame tant que le transport n'est pas ouvert (`client/realtime/RealtimeClient.ts:1148`) | Émettre après la résolution de `connect()`                                                   |
