...
// Par module (msgid)
syslog.listenWithConditions({ msgid: { data: "DATABASE" } }, (pdu) =>
  metrics.record(pdu),
);

// RegExp sur le msgid
syslog.listenWithConditions({ msgid: { data: /^NODEFONY/ } }, (pdu) => pdu);
...
// HTTP POST JSON
syslog.addTransport(
  new HttpTransport({ url: "http://logs.example.com/ingest" }),
);
