...
- **Override par module (à chaud, sans reboot)** : `_debugOverrides: Map<msgid,number>|null` (**lazy null = 0 coût hot path** ; re-null au dernier clear). `setDebugOverride(module, level, ttlMs?)` élève le seuil EFFECTIF d'UN module (clé = `msgid` du Pdu = nom du Service par défaut, cf `Service.log`) : `threshold = override ?? global`. Seuil **absolu** (peut aussi restreindre). `ttlMs` → auto-extinction (timer `unref` ré-armable). `clearDebugOverride`/`clearAllDebugOverrides`/`getDebugOverrides()`. Sans gate global (dev, `null`) → sans effet (tout passe déjà). Purgé par `reset()`.
