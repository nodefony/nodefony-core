...
**Gate d'entrée par sévérité (T2) + debug runtime PAR MODULE**

- `_severityThreshold: number|null` (`null`=pas de gate) — court-circuit AVANT alloc dans `log()` : `sev > threshold` → `gated++`, retourne le `_gatedPdu` singleton (`DROPPED`). Posé par le Kernel (`prod && !debug ? "INFO" : null`), re-résoluble à chaud `setSeverityThreshold(level|null)`. Probe publique `severityEnabled(sev)`.
- **Override par module (à chaud, sans reboot)** : `_debugOverrides: Map<msgid,number>|null` (**lazy null = 0 coût hot path** ; re-null au dernier clear). `setDebugOverride(module, level, ttlMs?)` élève le seuil EFFECTIF d'UN module (clé = `msgid` du Pdu = nom du Service par défaut, cf `Service.log`) : `threshold = override ?? global`. Seuil **absolu** (peut aussi restreindre). `ttlMs` → auto-extinction (timer `unref` ré-armable). `clearDebugOverride`/`clearAllDebugOverrides`/`getDebugOverrides()`. Sans gate global (dev, `null`) → sans effet (tout passe déjà). Purgé par `reset()`. Wildcard **`*`** = debug GLOBAL (un override spécifique prime ; réutilise le TTL). `getDebugOverrideExpiry()` = échéances epoch ms (countdown Studio).
- `severityFromInput(level): number|null` — validation **stricte** (nom RFC 5424 | 0-7) pour les surfaces d'entrée (endpoint/env), REJETTE l'inconnu (≠ `toSeverityNumber`, fail-open `-1` du hot path).
- `parseDebugSpec(spec)` — parse `NF__DEBUG` (`*`=global, `MOD`, `MOD:LEVEL`, séparés virgule/espace) → `{global, overrides}` ; appliqué par le Kernel au boot (lu direct `process.env.NF__DEBUG`, knob framework comme `NODE_ENV`). Endpoint admin : `PATCH /nodefony/kernel/api/log/level` (cf framework MEMORY).
...
- **WRITE↔READ cohérents** : `Kernel.initializeLog`, si `log.queryDriver:"file"` → enregistre le driver + branche un `FileTransport({path, format:"json"})` sur le même `log.queryFile.path` (défaut `logs/nodefony-<pid>.jsonl`). Opt-in (jamais le défaut) → l'écriture fichier assumée par dev/VPS, hot path cloud-native = stdout→Loki.
...
- **`createLokiLogDriver` / `createOpenSearchLogDriver`** (**Node-only**) : drivers PROD distants. Loki = query LogQL `GET /loki/api/v1/query_range` (sélecteur `{app=…}` + severity label + line filters `|=`/`|~`) ; OpenSearch = `POST /{index}/_search` (bool filter range timeStamp + sort desc + size). **Adaptateurs PAS moteurs** : le distant BORNE le rapatriement (range+limite+grep), puis **`filterPdus` = autorité finale** (sémantique identique memory/file). Lignes/`_source` = record wire → `coerceRecord` (réexporté de FileLogDriver) → `filterPdus`. Erreur réseau/5xx → `throw` (infra visible) ; OS 404 index absent → vide. Chacun expose **`probe()`** (loki `/ready`, opensearch `GET /`+`_count` → `{ok,latencyMs,info}`).
- **WRITE batché** = `LokiTransport` / `OpenSearchTransport` (`transports/`), étendent **`BatchingHttpTransport`** (base : ring borné + drop anti-OOM + flush seuil/intervalle/`beforeExit`, `send()` O(1) Promise résolue partagée, 1 POST/lot). Loki = `streams` groupés par labels (basse cardinalité app/severity/module/pid ; ts en **ns** = `ms·10⁶` par suffixe `"000000"`) ; OpenSearch = NDJSON `_bulk` (terminé par `\n`). `fetch` injectable (`httpFetch.ts` `FetchLike`/`resolveFetch`/`fetchWithTimeout`) → testable sans réseau (`tests/LogBackplaneHttp.test.ts`, fetch mocké).
- 🆕 **Registre de FABRIQUES** (`logDriverRegistry` : `registerLogDriverFactory(name, ctx→{driver,transport?,writeKey?})`, `builtinLogDrivers.ts` enregistre les 5 natifs). **Le Kernel ne hardcode plus aucun `if(name===…)`** : `initializeLog` monte `{memory, queryDriver}` + en dev **TOUS** les drivers enregistrés (`listLogDriverFactories()`, chacun s'auto-skippe `null` si config absente — loki/opensearch sans URL). `writeKey` déduplique le transport partagé (file↔cluster-file = même JSONL). Config `log.loki`/`log.opensearch` (env `NF_LOG_QUERY_DRIVER`/`NF_LOKI_URL`/`NF_OPENSEARCH_URL`). En **dev** = double-push (switch à chaud Studio loki↔opensearch sans reboot) ; en **prod** = opt-in strict (12-factor).
- **Sonde Studio** : `GET /nodefony/syslog/api/backplane/ping?driver=` (framework `SyslogAdminApi`, via `getLogDriver(name)` + `probe()`) → joignabilité/latence/infos. Drivers locaux (memory/file) sans `probe` → `{ok:true, latencyMs:0, local}`. **Validé runtime** contre vrais conteneurs (compose `docker/`, profils `--profile loki`/`--profile opensearch`, réseau `nodefony-net`).
...
- `FileTransport` : JSON ou text — `new FileTransport({ path, format?, maxPendingBytes? })`. UN `WriteStream` d'ajout, ouvert au 1ᵉʳ `send` (ordre garanti ; plus d'`appendFile` par ligne, qui entrelaçait les lignes concurrentes). File bornée 8 Mio : au-delà, lignes perdues + `dropped`, 1 rejet par épisode de saturation. `close()` idempotent, `send` après `close` rouvre. `Syslog.removeTransport` / remplacement par name → `close?()` (sinon fuite de fd à chaque `initializeLog`) ; échec → `onTransportCloseError(err, transport)` (contrat `onTransportError` = pdu non nul).
