...
export const NPM_SCRIPT_CATALOG: readonly {
...
}[] = [
  {
    ...
    group: "Qualité — la passe (npm run)",
  ...
  {
    script: "coverage",
    summary: "couverture des tests (v8)",
    group: "Qualité — la passe (npm run)",
    when: "La couverture réelle des tests. À lire comme une CARTE des zones jamais exercées, pas comme une note : 100 % de lignes couvertes ne dit rien des cas limites.",
  },
  {
    script: "format:check",
    summary: "le formatage est-il conforme ?",
    group: "Qualité — code (npm run)",
    when: "Contrôle le formatage SANS rien réécrire — c'est la version que la forge lance. `format` corrige, celui-ci constate.",
  },
  {
    script: "doc:anchors",
    summary: "les ancres fichier:ligne de la doc",
    group: "Qualité — doc et pilotage (npm run)",
    when: "Résout chaque ancre `fichier:ligne` de la documentation contre le code. Une ancre qui a dérivé est PIRE qu'une ancre absente : elle a l'air d'une preuve, et le lecteur croit la mauvaise ligne.",
  },
  {
    script: "doc:anchors:fix",
    summary: "recale les ancres qui ont dérivé",
    group: "Qualité — doc et pilotage (npm run)",
    when: "Recale les ancres PAR SYMBOLE, et seulement quand une seule définition plausible existe — le reste demande un jugement et reste signalé. À lancer après un refactor qui a décalé des lignes.",
  },
  {
    script: "doc:lint",
  ...
  {
    script: "check:lang",
    summary: "identifiants en anglais",
    group: "Qualité — code (npm run)",
    when: "Le code part sur npm et s'affiche dans l'autocomplétion de gens qui ne parlent pas français : ce gate refuse un identifiant français. La prose, elle, reste en français.",
  },
  {
    script: "doctor:live",
    summary: "diagnostic QUI INTERROGE l'app",
    group: "Avant de déployer (npm run)",
    when: "Le diagnostic statique ne lit que des fichiers ; celui-ci DEMANDE à l'application — verdict des migrations, cohérence des zones du firewall. Deux vérités qui ne sont dans aucun fichier. Il boote, sans ouvrir de port.",
  },
  {
    script: "audit:deps",
    summary: "vulnérabilités qui atteignent la prod",
    group: "Avant de déployer (npm run)",
    when: "`npm audit` borné aux dépendances de PRODUCTION : celles qui partent réellement dans l'image. Les outils de développement y ajoutent du bruit qu'aucun utilisateur n'exécute.",
  },
  {
    script: "ticket:lint",
    summary: "cohérence du tableau de bord",
    group: "Qualité — doc et pilotage (npm run)",
    when: "Confronte les tickets à leurs propres règles : jalon promis mais absent du tableau, ordre en double, dépendance inversée, statut qui ment. Un ticket hors tableau n'entre dans AUCUN compteur, et rien ne le dit.",
  },
  {
    script: "infra:up",
