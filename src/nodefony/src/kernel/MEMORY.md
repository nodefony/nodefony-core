...
- **Registre de DISPONIBILITÉ** (`readinessRegistry.ts` + `Kernel.setReadiness/clearReadiness/readinessBlocked/readinessReport`) : un composant qui a un état d'amorçage (schéma de base en retard, cache froid, service tiers muet) RETIENT la mise en service. `/readyz` rend 503 tant qu'un contributeur répond `false` ; `/livez` reste 200 — un état EXTERNE ne se répare pas en redémarrant le process, et un `livez` qui tomberait déclencherait une cascade de redémarrages. **Le verdict est DÉJÀ calculé** : la sonde lit un entier, elle ne déclenche aucune vérification (une sonde qui interroge une base tombe avec elle) ; le contributeur le repose à son rythme, et le pod redevient disponible SEUL — sans redéploiement. **Lazy** : le registre est `null` tant que personne ne s'inscrit et REDEVIENT `null` quand le dernier se retire (la sonde est sur le chemin le plus chaud). Idempotent par nom (un même nom = une voix), un seul non-prêt suffit à retenir. Journal : CRITIC à la bascule, avec les noms et les raisons — le corps de la sonde, lui, reste une constante pré-allouée. **Une seule vérité** : `livez.ready` (data plane) vaut `booted && readinessBlocked === 0` et publie `readinessBlocked` ; `nodefony status` affiche une ligne `disponibilité` (sondée via `probeReadiness`, jamais sur un port tenu par un AUTRE projet). **Et il NOMME ce qui retient** : le noyau publie `{pid, contributors, ts}` dans `node_modules/.cache/nodefony/readiness.json` (`writeReadinessState`, à côté du pidfile et de `runtime.json`), `status` le lit (`readReadinessState`). 🔴 **Fichier et non sonde** : `livez` ne rend les noms qu'à un appelant authentifié — la gradation est délibérée (compte public, noms réservés) et `status` interroge en anonyme ; le système de fichiers EST la frontière « local », meilleure qu'une adresse d'appel (un conteneur voisin présente la boucle locale), et il répond quand la boucle d'événements ne répond plus. Écriture **seulement si la signature de l'état change** (un contributeur qui repose le même verdict toutes les 15 s ne touche pas le disque), effacement quand le dernier contributeur part, best-effort partout (FS en lecture seule → on retombe sur le compte). 🔴 **Deux sources doivent CONCORDER pour nommer** (`withBlockedBy`) : le compte vient du runtime interrogé à l'instant, le détail d'un fichier qui peut avoir un cycle de retard — s'ils divergent, on garde le compte nu, un nom faux envoyant chercher une cause déjà levée. ⚠️ Ne pas confondre avec `extractSkippedModules`, qui contourne la même gradation en relisant le JOURNAL : là c'est légitime (un module écarté est un fait de BOOT, écrit une fois et toujours dans le journal récent) ; la disponibilité est un état VIVANT qui dure des heures — un journal qui a tourné ne le dirait plus. Le CODE HTTP de `livez`, lui, reste piloté par le seul `booted` : il sert la chaîne de démarrage (`probeBootDegraded`), et un pod volontairement retenu est bel et bien DÉMARRÉ.
