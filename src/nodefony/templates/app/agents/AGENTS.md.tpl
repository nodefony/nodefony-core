...
| Rends **Vite joignable** — ou passe en pré-bâti                      | En mode Vite la page annonce ses assets sur `127.0.0.1:5173` : l'adresse du NAVIGATEUR, donc du conteneur, où aucun Vite ne tourne. Écran blanc, HTML pourtant servi. Remède avec HMR : `NF_FRONTEND_PUBLIC_ORIGIN=https://host.docker.internal:{port}` (l'origine des assets, l'allowlist Vite et le WebSocket HMR suivent, `{port}` = port réel). Sans HMR : servir le pré-bâti. ⚠️ Cette variable est un décor d'OBSERVATION : restée posée, elle casse le navigateur du POSTE (qui ne résout pas `host.docker.internal`) — retire-la et relance le serveur quand tu as fini d'observer. |
