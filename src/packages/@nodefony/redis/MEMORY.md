...
- Env (dans builder, après parse): `REDIS_URL`>host/port/auth, `NF_REDIS_HOST`, `NF_REDIS_PORT` (validé), `NF_REDIS_PASSWORD`.
...
`npx vitest run` (gate serveur réel : `NF_REDIS_TEST_URL=redis://:<pass>@127.0.0.1:6379/15`) — compte réel = `npx vitest run 2>&1 | tail -3`. Couverture par fichier :

- **unit/config.test.ts** (10) : schema défauts, env layering, buildClientOptions merge/url/reconnect. Sans serveur. **Purge REDIS\_\* env au chargement** (isolation, sinon `NF_REDIS_PASSWORD=... vitest` casse le test password).
...
- **integration/session-pagination.test.ts** : banc de contrat de pagination de `@nodefony/http` en mode **curseur**. Double déterministe par défaut, vrai serveur si `NF_REDIS_TEST_URL`.
- **integration/{token-store,token-pagination,webauthn-credential-store}.test.ts** : stores security.

Infra : `docker compose -f docker/docker-compose.yml up -d` (password `nodefony-dev`). Lancer : `NF_REDIS_PASSWORD=nodefony-dev npx vitest run` (ou défaut nodefony-dev si env absent).
