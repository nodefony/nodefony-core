...
/**
 * Schéma Zod de la configuration de `@nodefony/redis`.
 *
 ...
 * builder {@link defineRedisConfig}) → plante propre avec messages clairs si la
 * config est invalide, plutôt qu'un `undefined.x` silencieux en runtime.
 *
 * Convention figée 2026-05-28 (cf mémoire `feedback_config_validation_zod`).
 * Alignement sur `@nodefony/realtime/nodefony/config/schema.ts` et
 * `@nodefony/security/.../defineSecurityConfig.ts`.
 *
    ...
    url: z
      ...
      .describe(
        "URL Redis complète `redis[s]://[[user][:pass]@]host[:port][/db]`. Si " +
          ...
          "extraits de l'URL). Pratique pour les PaaS (Heroku, Upstash…).",
      ),
