...
- **`connectionMonitor`** : `recordLost(name, reason?)`/`recordReconnect` = compteurs CONSTATÉS, DATÉS (`lastLostAt`/`lastRestoredAt`) + chronologie `events` (ring 20, lazy, plus récents d'abord) servie par `connection/health`. `Orm.describeResilience()` (optionnel sur `IOrm`) = `{heartbeatMs, heartbeatTimeoutMs, heartbeatActive, pingable, lostPending}` → champ `resilience` du même endpoint (lu par la page Studio d'un connecteur). `reconnectCount` n'est PLUS dérivé de `connectCount-1` (une reprise de driver ne repasse jamais par `connect()`, et 2 connecteurs comptaient une reconnexion fantôme). `connectedSince` retombe à `null` à la perte — un uptime qui court pendant une coupure se lit comme une preuve de santé.
...
- `ITransaction`: commit/rollback/savepoint/rollbackTo/getNative<C>. Driver sans savepoints (Mongo) → `SavepointNotSupportedError` (jamais no-op).
