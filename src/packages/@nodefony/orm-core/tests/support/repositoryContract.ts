 ...
 * exécutée sur chaque adaptateur via un harnais (Drizzle × sqlite/postgres/
 * mysql, Mongoose × MongoDB). Vit chez le propriétaire du contrat
 * (`@nodefony/orm-core`) : deux copies divergeraient en silence, chacune
 * passant ses propres tests.
 *
 * **Pourquoi ce banc existe** : les chemins d'exécution divergent radicalement
 * par moteur (RETURNING vs re-SELECT-par-PK, `ON CONFLICT` vs `ON DUPLICATE KEY
 * UPDATE` vs `findOneAndUpdate`, `limit(-1)` vs OFFSET seul vs sentinel…) — la
 * seule preuve que le CONTRAT est identique, verbe par verbe, est de faire
 * passer les mêmes assertions à tous les backends. Un développeur
 * d'application qui change `NF_DATABASE_URL` ne doit observer AUCUNE
 * différence de comportement : chaque écart trouvé ici est un bug du
 * framework, pas de l'app.
 *
 * Divergence sémantique ASSUMÉE (non testée, documentée) : la sensibilité à la
 * casse de `$like` suit la collation du backend — le banc n'utilise que des
 * motifs à casse exacte.
 *
 * **Le harnais déclare l'entité sonde** ({@link PROBE_ENTITY}, forme
 * {@link ProbeRow}) dans le dialecte de son moteur, avec trois défauts posés
 * par l'ADAPTATEUR à la création : `id` (UUID texte), `active` (`true`),
 * `createdAt` (epoch ms courant).
 */

/** Nom logique de l'entité sonde, que chaque harnais déclare. */
export const PROBE_ENTITY = "repo_contract_probe";

/** Forme d'une ligne de l'entité sonde (tous les types du contrat). */
...
/** Branchement d'un adaptateur sur le banc. */
export interface RepositoryContractHarness {
  /** L'ORM CONNECTÉ, l'entité sonde déjà déclarée (résolu paresseusement). */
  orm: () => IOrm;
  /**
   * Un ORM du même adaptateur, JAMAIS connecté, et de quoi le désinscrire —
   * prouve qu'une transaction hors connexion lève au lieu de se taire.
   */
  offline: () => { orm: IOrm; dispose: () => void };
  ...
  savepoints: boolean;
  /**
   * Assertions de sonde propres au moteur (stockage d'un fichier, pool d'un
   * serveur) — ce que `probe()` doit REFLÉTER, au-delà de ne pas être vide.
   */
  assertProbe?: (orm: IOrm) => Promise<void>;
}

/**
 * Déroule la suite de contrat sur l'adaptateur branché. À appeler DANS un
 * `describe` (éventuellement `describe.skipIf(!url)`) du fichier consommateur,
 * APRÈS le `beforeAll` qui connecte l'ORM.
 */
export function runRepositoryContract(
  ...
  beforeAll(async () => {
    orm = harness.orm();
    repo = orm.getRepository<ProbeRow>(PROBE_ENTITY);
    await repo.delete({}); // base persistante entre les runs
  });
  ...
  it("find : $null / valeur nue null — « la colonne est vide » (jamais eq(col, NULL))", async () => {
    ...
    assert.deepEqual(
      (await repo.find({ note: { $nin: ["n1"] } })).map((r) => r.name),
      ["chloé 👩‍💻"],
      "`$nin` n'attrape pas davantage les NULL",
    );
    // Un `$nin` VIDE n'exclut rien — NULL compris : il reste neutre.
    assert.equal((await repo.find({ note: { $nin: [] } })).length, 4);
  });
  ...
  it("transaction : hors connexion → `not connected` (jamais un silence)", async () => {
    const { orm: offline, dispose } = harness.offline();
    try {
      await assert.rejects(
  ...
  it("expose TOUTE l'introspection (ping, probe, describeConnection, describeEntity)", () => {
    // Optionnelles dans `IOrm`, exigées ici : un adaptateur qui ne les porte
    // pas laisse le panneau ORM de la console muet, sans une erreur.
    assert.equal(typeof orm.ping, "function");
    assert.equal(typeof orm.probe, "function");
    assert.equal(typeof orm.describeConnection, "function");
    assert.equal(typeof orm.describeEntity, "function");
  });
  ...
  it("describeConnection : driver = dialecte, cible renseignée, ZÉRO credential", async () => {
    ...
    assert.equal(info.driver, harness.driver);
  ...
  it("describeEntity : colonnes normalisées (alimente l'ERD / l'IA du data plane)", async () => {
    const cols = orm.describeEntity!(PROBE_ENTITY);
    ...
    assert.ok(byName.get("age")?.type, "type natif du moteur");
    assert.deepEqual(orm.describeEntity!("ghost"), [], "entité inconnue → []");
  });

  it("probe : JAMAIS muette sur un connecteur connecté", async () => {
    const p = await orm.probe!();
    assert.ok(
      Object.keys(p).length > 0,
      "sonde vide = panneau Studio ORM muet en production",
    );
    // Ce que la sonde doit REFLÉTER dépend du moteur (stockage, pool…) : le
    // harnais le porte, le banc garantit qu'elle n'est jamais vide.
    await harness.assertProbe?.(orm);
  });
}
