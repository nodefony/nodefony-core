...
> + classes de base, partagés par les drivers (`@nodefony/sequelize`,
> `@nodefony/mongoose`, `@nodefony/drizzle`…). **Lib pure** : pas de `Module`
> runtime, pas dans `@modules()` — ce sont les drivers qui sont des modules.

## Quel ORM par défaut ?

**Aucun n'est imposé** : orm-core est une abstraction *Repository*. On choisit l'ORM
...
en `Op.*` Sequelize, `$`/`$regex` Mongoose, `eq()/gt()/inArray()/like()` Drizzle) :
...
> `$like` est une **sémantique SQL** : native pour Sequelize/Drizzle, traduite en
...
**déclarées** dans `@entity` (`include` Sequelize / `populate` Mongoose /
...
(Sequelize/Mongoose/Drizzle, P5.4 + P7.4), opérateurs riches (P7.4). Les **4 risques
