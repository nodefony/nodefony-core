...
- **Dashboard** : `/info` réel = `/nodefony/kernel/api/info` (plus le mock studio ; expose `debug`, `version`, `node`...) + KPI **Routes** réel = `/nodefony/framework/api/info` `routesTotal`. Stats live = WS `dashboard:stats` (inchangé). Sessions KPI = stub (pas d'endpoint).
