...
- **Modules** (`routes/Modules.tsx`, route `/nodefony/modules`) : page RÉELLE (remplace le stub) inspirée du legacy `monitoring-bundle/views/bundles/Bundle.vue` — **grille de cartes** (1 par module : icône app/package, nom, version, type, chemin mono, dépendances en chips orange pour `@nodefony/*`). Liste `/api/modules` + détails `/api/module/{key}` en parallèle.
