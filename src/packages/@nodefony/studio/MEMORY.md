...
- **ModuleDetail** (`routes/ModuleDetail.tsx`, route `/nodefony/modules/:name`) : page détail à **onglets Mantine** (comme le détail legacy `Bundle.vue`) : Vue d'ensemble · Dépendances · **Routes** (table réelle filtrée par `route.module === key` depuis `/framework/api/routes`) · Config/Services (stubs P10.4/P10.10). Détail via `/api/module/{key}`. Jointure routes : `route.module` = **clé courte** du module (app/framework/studio/test/test-frontend-react), pas le nom `@nodefony/*`.
