...
- `StudioController` `@controller("/nodefony")` : UI = `/nodefony` (HTML+renderTags) + `/nodefony/{page}` (SPA fallback 1 seg) + **`/nodefony/{section}/{page}` (SPA fallback 2 seg — deep-link/F5 sur `modules/:name`, fix 404 2026-05-20 ; sûr car routes admin ≥3 seg `/api/*`, SPA pas de route 3+ seg)** ; API = `/nodefony/studio/api/{health,info}`, `/nodefony/studio/api/auth/{login,me,logout}` (MOCK), `/nodefony/studio/api/realtime/info` (available:true), `/nodefony/studio/api/logs/stream` (SSE Pdu — **dormant**, front passé en WS).
