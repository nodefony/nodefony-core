...
│   ├── controller/StudioCreateController.ts    ← matière du formulaire « Créer » (spec du moteur, cibles, racines) — 403 hors dev
│   ├── service/ScaffoldService.ts              ← pilote `nodefony create` : preview (dry-run), job streamé, étapes npm en allowlist
...
| --- | --- | --- | --- |
| `/studio/api/health` · `/info` (bypass) · `/stats` (`ROLE_SUPERVISOR`, **GET + WEBSOCKET**) | GET | Ping public / infos minimales pré-login (`/info` réduit name/version/env — plus de version Node/mémoire/debug) / snapshot sondes process (gardé RBAC). ⚠️ `/stats` est appelée par la console **via le pont `api.request`** du socket : le routeur résout alors sur le transport `WEBSOCKET`, qu'une route doit DÉCLARER — sans quoi 405 « Method WEBSOCKET Unauthorized », invisible côté HTTP. Le transport dit par où l'on entre, jamais qui a le droit : `@IsGranted` reste évalué (un `ROLE_USER` reçoit 403). Gardé par `nodefony/tests/unit/routesPontables.test.ts` | kernel |
...
| `/nodefony/studio/api/realtime` | **WS** | **WebSocket permanent JSON-RPC 2.0** (`StudioRealtimeController`) — pub/sub par canal (`subscribe`/`unsubscribe` : `nodefony:syslog`, `nodefony:dashboard`, …) **+ actions requête→réponse** (`nodefony:kernel:{ping,gc}`, `nodefony:scaffold:{preview,run,cancel}`) | RealtimeService P13.4 |
| `/nodefony/studio/api/create/{spec,browse,job/{id},job/{id}/archive}` | GET | Matière du formulaire « Créer » : spec du MOTEUR en JSON, questions `optionsFrom` HYDRATÉES (`hydrateQuestion`), + cibles, racines autorisées, capacités et **contexte projet** (connecteurs réels, entités, `columnTypes` des quatre moteurs → panneau « Types de champ »). Composée par `src/createSpec.ts` (`composeCreateSpec`), éprouvée sur une app SQLite ET une app MongoDB ; explorateur borné, état d'un job, archive d'une app générée. **403 hors développement**, quel que soit le rôle — ces routes écrivent sur le disque et lancent `npm`. L'exécution, elle, passe par le temps réel (voir ci-dessus) | — |
...
- **Actions (requête→réponse)** : une frame AVEC `id` est une **requête RPC** qui attend une réponse `result`/`error` (≠ pub/sub). Routées par `dispatchRequest()` → méthode connue renvoie `{jsonrpc,id,result}`, inconnue `-32601`, handler qui throw `-32603` (message **générique** au client, détail loggé serveur = Zero Trust). Méthodes : **`nodefony:kernel:ping`** (liveness + RTT, lecture pure), **`nodefony:kernel:gc`** (force GC si `--expose-gc`, sinon `{available:false}` ; action de contrôle à effet réel), et le générateur de code **`nodefony:scaffold:{preview,run,cancel}`** — `preview` rend le PLAN (fichiers créés, diff des réécritures) sans rien écrire, `run` démarre un job dont la progression est streamée sur `nodefony:scaffold:job@<id>`. La liste canonique est `PLATFORM_METHODS` (core) : y ajouter une entrée sans la marque `nodefony:` casserait le plancher de sécurité en silence (un test le verrouille). Le chemin chaud subscribe reste sync/0-alloc — le coût n'est payé que sur une requête `id`. Côté client = `RealtimeClient.request(method,params)` (Promise id-matchée) + helper réutilisable **`client.ping()`** (mesure le RTT). Forward-compat P13.4 : routeur + actions migrent tels quels dans `RealtimeService`. ⚠️ **réutilisable = dans la lib cliente** (`nodefony`), jamais dupliqué par front.
