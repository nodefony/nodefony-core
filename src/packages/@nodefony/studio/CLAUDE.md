...
| `/nodefony/studio/api/realtime`                                        | **WS**  | **WebSocket permanent JSON-RPC 2.0** (`StudioRealtimeController`) — pub/sub par canal (`subscribe`/`unsubscribe` : `nodefony:syslog`, `nodefony:dashboard`, …) **+ actions requête→réponse** (`nodefony:kernel:ping`, `nodefony:kernel:gc`) | RealtimeService P13.4 |
