...
| `/nodefony/studio/api/create/{spec,browse,job/{id},job/{id}/archive}`                       | GET     | Matière du formulaire « Créer » : spec du MOTEUR en JSON (+ cibles, racines autorisées, capacités), explorateur borné, état d'un job, archive d'une app générée. **403 hors développement**, quel que soit le rôle — ces routes écrivent sur le disque et lancent `npm`. L'exécution, elle, passe par le temps réel (voir ci-dessus)                                                                                                                                                                                                                                        | —                     |
