...
- **Erreurs** — une seule, `RpcError` (core, `realtime/JsonRpcPeer.ts:71`), isomorphe : `code` NUMÉRIQUE JSON-RPC + `data` qui traversent le fil. Pas d'erreur maison dans le module : un `*Error` exporté sans site d'émission est refusé par `tests/unit/errorSurface.test.ts`. Action qui lève autre chose → `-32603` opaque (Zero Trust). Erreur HTTP-like (`@IsGranted` 403) → `-32000` + `data.status` via `toFrameRpcError` (`RealtimeController.ts:83`).
...
defineRealtimeConfig({
  ...
  clientLogs: { enabled: false }, // canal MONTANT des journaux navigateur — FERMÉ par défaut
});
...
- **Journaux du NAVIGATEUR (canal montant `nodefony:syslog:uplink`)** : surface d'ÉCRITURE, donc fermée par défaut (`clientLogs.enabled`). Les bornes voyagent config → `RealtimeService.init()` → `hub.setClientLogsLimits()` → `RealtimeController` au handshake (même patron qu'`originGuard`/`maxChannelsPerConnection`/`slowConsumerBytes`). `null` sur le hub ≠ « handler qui refuse » : AUCUN handler n'est déclaré, la frame est droppée comme une méthode inconnue. Déclaré par la BASE (pas par un contrôleur nommé) pour servir une application quelconque ; un `realtimeInbound()` qui déclare le même nom GAGNE. Handler NEUF par connexion ⇒ compteur de débit per-connexion sans registre partagé. Trois bornes, une par test : origine FORCÉE (`BROWSER_ORIGIN` du cœur, jamais le `moduleName` du client ; le `Pdu` est construit à la main car `syslog.log(payload,…)` prendrait le module de SES réglages) · taille+débit bornés · sévérité plafonnée à ERROR. Le `requestId` d'une entrée est DÉCLARÉ par le client, validé en forme seulement → piste de diagnostic falsifiable, jamais élévation de droit. Politique : règle système DÉDIÉE nommée avant le générique (`UPLINK_CHANNEL_POLICY` = `authenticated` seul) — `SYSTEM_CHANNEL_POLICY` (ROLE_ADMIN) protège la LECTURE de l'état du pod, un canal montant ne rend rien. Conséquence : une connexion ANONYME ne pousse rien.
...
- **`realtime:welcome` n'annonce QUE les canaux que CE visiteur pourrait obtenir.** `announcedChannels` est filtré par `hub.probeChannel(channel, peer)` (`src/server/RealtimeController.ts`), qui appelle la **sonde muette** — le MÊME verdict que le verrou de `subscribe`, sans rapporteur d'audit. 🔴 Pourquoi une SECONDE closure : passer par `runAuthorizer` écrirait N entrées « frame.denied » au journal d'audit à CHAQUE connexion, pour des demandes que personne n'a faites. Elle se pose par le MÊME appel (`setFrameAuthorizer(authorizer, { silentProbe })`) pour qu'aucun câblage ne puisse poser l'un sans l'autre — la dégradation serait muette (l'annonce redeviendrait entière, sans erreur). Sans sécurité chargée, aucune sonde → rien n'est retiré (échec OUVERT : c'est le verrou qui protège l'accès, pas l'annonce). Coût mesuré : **0,55 µs par connexion** pour 6 canaux, jamais par frame. Un cas confronte les deux verdicts canal × identité (`tests/integration/realtimeChannelAuth.e2e.test.ts`) — deux copies d'une règle divergent sinon en silence.
- **`realtime:welcome` porte le MODE du serveur** (`env`, contrat `IRealtimeWelcome` du cœur) — **posé UNIQUEMENT hors production**, par la fonction pure `src/server/welcomeEnv.ts`. Le client s'en sert pour décider s'il peut parler dans la console : `import.meta.env.DEV` ne dit que le mode du BUNDLE, et une app bâtie en production servie par un serveur de dev (banc, cluster local) restait muette. 🔴 Ce n'est pas un réglage qu'on lit, c'est une **permission de parler** : une absence vaut production, jamais l'inverse. La règle est isolée dans sa fonction parce que, noyée dans la composition du handshake, son inversion ne casserait aucun test et annoncerait le mode d'un serveur publié à ses visiteurs. Banc `tests/unit/welcomeEnv.test.ts` (4, garde vue rouge en débranchant `env === "production"`).
...
- **Aucun `subscribe` ne reste sans réponse** (`RealtimeController.startChannel`) : trois motifs de `realtime:denied`, et rien qui tombe entre — `forbidden` (plancher de plateforme, ou verrou de frame par `onFrameAudit`), `limit` (plafond de canaux de la connexion), `unknown` (le hub n'a trouvé AUCUN producteur : nom mal orthographié, module non chargé). Contrat fermé côté cœur : `RealtimeDeniedReason` (`nodefony`), normalisé à l'ingestion cliente (motif inconnu → `forbidden`). `unknown` n'ouvre pas d'oracle : un canal gardé est tranché en amont, donc rendu `forbidden` qu'il existe ou non.
...
(`tests/integration/fanoutSerialize.perf.test.ts`, opt-in `NF_RUN_PERF=1`), pas le système saturé.
...
- **`onPlatformNotice` est posé APRÈS le handshake** : le controller pose le sien pendant
  (`RealtimeController.ts:575`) et l'écraserait. Les avertissements utiles (canal dynamique
