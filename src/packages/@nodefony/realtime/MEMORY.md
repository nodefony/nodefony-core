...
- **Env layering** `NODEFONY_REALTIME_DRIVER` (convention-frère `REDIS_*`) surcharge le driver après parse dans `defineRealtimeConfig`. ⚠️ L'override app `module-realtime` n'atteint PAS realtime (validé à onRegister, override appliqué à onPreBoot → trop tard : chantier config ordering). Env = lever fiable en attendant.
...
- **🔴 DETTE #1 namespace topic non câblé** : `REDIS_RT_CHANNEL="nodefony:realtime"` en dur ; la factory prod (`index.ts#wireBackplane`) ne passe PAS le 3e arg du constructeur ; `backplaneSchema` n'a que `driver` (pas de champ namespace) → **cross-talk multi-app** sur Redis/Kafka mutualisé (le `database` Redis NE cloisonne PAS le pub/sub, global). Fix : champ config OU dériver auto de `kernel.name`. Détail : `docs/realtime/socket/08-distribue.md`.
- **🔴 DETTE #2 (HAUTE) `originId = String(process.pid)` non unique en k8s** : namespace PID par conteneur → 2 pods peuvent avoir le même pid (PID 1) → `#ingress` filtre `msg.originId === this.originId` → **fan-out cross-pod jeté SILENCIEUSEMENT** (anti-echo confond les pods). OK en cluster mono-host (pid réels distincts). Fix : `process.env.POD_NAME ?? os.hostname()+":"+pid ?? randomUUID()`.
