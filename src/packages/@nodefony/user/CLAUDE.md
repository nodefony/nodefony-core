...
- ✅ **P5.5a** scaffold workspace (ce commit) : package.json/tsconfig/rollup/index.ts/docs + arbo `nodefony/{contracts,src/encoders,service}/`.
- ⬜ **P5.5** contracts : `IUser`, `IRole`, `IPermission`, `IUserProvider`, `IUserRepository`, `IPasswordEncoder` + `BaseUser` + `AnonymousUser`.
