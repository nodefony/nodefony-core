...
- `nodefony/src/password/` : `PasswordPolicy` (impl `IPasswordBlocklist`) **posée par DÉFAUT** sur `UserService.passwordBlocklist` — 6 règles ordonnées (coût croissant) : `minLength` (10, source UNIQUE du plancher, `UserAdminApi` et l'écran de profil Studio en DÉRIVENT) · contient l'identifiant (partie locale d'un courriel, ≥ 4 car.) · motif répété (unité ≤ 4) · suite de touches (azerty + qwerty + qwertz, tranches de 4, deux sens) · `blocklist`/`blocklistFile` de l'app · ~10 000 empreintes embarquées. `violation()` rend la RÈGLE en français, `isBlocked()` le booléen du contrat — une seule implémentation, deux portes. `commonPasswordHashes.ts` = **GÉNÉRÉ** (`scripts/generate-password-blocklist.mjs`, corpus SecLists MIT) : SHA-1 tronqué 4 octets, `Uint32Array` trié, recherche binaire, chargé par `import()` DYNAMIQUE au 1ᵉʳ contrôle (40 Ko jamais lus si l'app ne crée aucun compte). `truncatedPasswordHash` ≠ hachage de stockage (ni lent ni salé) — jamais pour un credential persisté. `blocklistFile` introuvable = erreur FRANCHE (un contrôle muet est pire qu'absent). `passwordBlocklist = null` désactive tout : geste explicite, plus un défaut invisible.
...
- **`USER_COLUMNS` (`src/userContract.ts`) = LE contrat de colonnes**, neutre vis-à-vis du moteur : par colonne son nom, son type logique (`uuid|string|string[]|boolean|object|object[]|date`), sa nullabilité/unicité, son `origin` (`identity` = clé primaire, `column` = colonne ordinaire — la seule que tout adapter DOIT déclarer, `audit` = horodatage), sa fabrique de défaut (`makeDefault`, jamais une valeur : un `[]` partagé contaminerait tous les comptes), `refreshedOnWrite` (déclaré, pas déduit du nom), et **`readers`** = les lecteurs qui cassent si elle manque, pour qu'un refus nomme la colonne ET son lecteur. `userTable` (drizzle) et `userSchema` (mongoose) en DÉRIVENT — 0 recopie, un test de parité par adapter. **Noms figés en v1** (le SQL natif du listing les écrit en dur ; desserrer est additif, resserrer est une rupture). **`USER_TABLE_NAME` (même fichier) = le nom de la TABLE**, membre du contrat au même titre : `queryKit` (SQL natif), `userTable` (spec colKit) et le générateur d'entité le LISENT — trois copies divergeaient, et celle du générateur disait `users`. Une divergence ne lève pas : la recherche par compte externe rend zéro ligne, donc un compte de plus à chaque connexion (banc `userTableNameParity.test.ts`).
- **`attachExtraColumns(user, row, skip?)`** (même fichier) = LE report des colonnes que `BaseUser` ne reconstruit pas — horodatages de la ligne ET **champs métier de l'app**. Appelé par les TROIS dépôts (drizzle, mongoose, mémoire) : une copie par dépôt divergerait (c'était le cas — SQL reportait les horodatages, Mongo non). Critère = `origin !== "audit"` ⟺ porté par `BaseUser` (invariant gardé par `tests/unit/userContract.test.ts`). N'alloue rien (`for...in`, hot path : 1 SELECT User par requête authentifiée). Refuse `__proto__`/`constructor`/`prototype` — un document Mongo peut porter ces clés. `skip` = plomberie du moteur (`_id`, `__v` côté mongoose), que le contrat ne peut pas connaître.
- **Écriture d'un champ métier : la porte est le dépôt GÉNÉRIQUE** (`orm.getRepository("User")`), pas `IUserRepository` — typé sur le contrat, il refuse le champ en TS. Documenté dans le TSDoc d'`IUserRepository` (il traverse jusqu'au `.d.ts`, donc l'IDE l'affiche au refus).
...
- **`changePassword` CHARGE le compte avant de juger** (`findById`) : la politique doit pouvoir
  refuser un mot de passe qui reprend l'identifiant, comme à la création — sinon un utilisateur
  contourne la règle en deux gestes (créer sain, puis remplacer par son nom). Effet de bord : un id
  inconnu rend `null` sans payer un hachage.
- **Le plancher de longueur n'a qu'UNE source** : `DEFAULT_PASSWORD_POLICY.minLength`. L'écran de
