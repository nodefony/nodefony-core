...
import { MemoryShutdownError, MemoryInvalidInputError } from "../src/errors/MemoryErrors.js";
...
const createMockLLM = (): ILLMProvider => ({
  name: "ollama", model: "test", mode: "sovereign",
...
describe("MemoryService", () => {
  ...
  describe("remember", () => {
    it("creates entry with id and timestamp", async () => {
      const entry = await memory.remember({
        agentId: "a1", sessionId: "s1", role: "user", content: "hello",
      });
    ...
    it("rejects empty agentId", async () => {
      await expect(memory.remember({
        agentId: "", sessionId: "s", role: "user", content: "x",
      })).rejects.toThrow(MemoryInvalidInputError);
    });

    it("rejects empty content", async () => {
      await expect(memory.remember({
        agentId: "a", sessionId: "s", role: "user", content: "",
      })).rejects.toThrow(MemoryInvalidInputError);
    });
    ...
    it("rejects too long content", async () => {
      ...
      await expect(memory.remember({
        agentId: "a", sessionId: "s", role: "user", content: huge,
      })).rejects.toThrow(MemoryInvalidInputError);
    });

    it("rejects invalid role", async () => {
      await expect(memory.remember({
        agentId: "a", sessionId: "s", role: "invalid" as never, content: "x",
      })).rejects.toThrow(MemoryInvalidInputError);
    });
  });
  ...
  describe("recall", () => {
    it("returns recent entries for session", async () => {
      await memory.remember({ agentId: "a", sessionId: "s1", role: "user", content: "msg1" });
      await memory.remember({ agentId: "a", sessionId: "s1", role: "assistant", content: "msg2" });
      const recalled = await memory.recall("a", "s1");
      expect(recalled.length).toBe(2);
    });

    it("isolates by agent", async () => {
      await memory.remember({ agentId: "a", sessionId: "s", role: "user", content: "hi" });
      await memory.remember({ agentId: "b", sessionId: "s", role: "user", content: "hi" });
    ...
    it("respects limit", async () => {
      for (let i = 0; i < 10; i++) {
        await memory.remember({ agentId: "a", sessionId: "s", role: "user", content: `msg${i}` });
      }
    ...
    it("rejects invalid limit", async () => {
      await expect(memory.recall("a", "s", 0)).rejects.toThrow(MemoryInvalidInputError);
      await expect(memory.recall("a", "s", 1001)).rejects.toThrow(MemoryInvalidInputError);
    });
  });

  describe("consolidate", () => {
    it("moves session entries to long term", async () => {
      await memory.remember({ agentId: "a", sessionId: "s", role: "user", content: "remember me" });
  ...
  describe("forget (RGPD)", () => {
    it("deletes all data for an agent", async () => {
      await memory.remember({ agentId: "a", sessionId: "s1", role: "user", content: "x" });
  ...
  describe("memory safety — InMemoryStore bounds", () => {
    it("enforces maxEntriesPerSession", async () => {
      ...
      try {
        for (let i = 0; i < 20; i++) {
          await m.remember({ agentId: "a", sessionId: "s", role: "user", content: `msg${i}` });
        }
    ...
    it("rejects after shutdown", async () => {
      await memory.shutdown();
      await expect(memory.remember({
        agentId: "a", sessionId: "s", role: "user", content: "x",
      })).rejects.toThrow(MemoryShutdownError);
    });

    it("clears all data on shutdown", async () => {
      await memory.remember({ agentId: "a", sessionId: "s", role: "user", content: "x" });
      await memory.shutdown();
