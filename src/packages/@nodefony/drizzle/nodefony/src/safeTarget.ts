 ...
 *
 * @param target - dialecte et coordonnées, telles que la résolution les rend.
 * @param cwd - répertoire de référence pour relativiser un chemin de fichier.
 * @param grammar - grammaire de chemins à employer. Injectée pour une seule
...
export function describeTargetSafely(
  ...
  if (target.dialect === "sqlite") {
    ...
    return relatif && !relatif.startsWith("..")
      ? relatif.split(grammar.sep).join("/")
      : grammar.basename(file);
  }
  const defaultPort = target.dialect === "postgres" ? "5432" : "3306";
