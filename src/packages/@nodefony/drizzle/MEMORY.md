...
- **Approche B** (PAS d'auto-register) : app câble `registerAuditStore("drizzle", ({container})=>DrizzleAuditStore.from(container.get("drizzle").getOrm("default")))` (registre @nodefony/security) + `registerAuditEntities("default")` avant connect + `security.audit.driver="drizzle"`. Défaut `memory` (registry `auditStoreRegistry`).
...
- **`connector.dialect`** (`sqlite|postgres|mysql`, défaut sqlite) + `url` (pg/mysql) dans `config/schema.ts`. Type `SqlDialect` exporté. `DrizzleOrmOptions` += `dialect`/`url`.
