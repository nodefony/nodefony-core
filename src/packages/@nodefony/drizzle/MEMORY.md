...
- **AUTO-REGISTER** (`registerStores.ts`) : entité + fabrique (registre @nodefony/security) déclarées par le module. Sélection = `security.audit.driver="drizzle"` SEUL (rétention lue de `config.audit.retentionDays`). Défaut `memory` (registry `auditStoreRegistry`).
