...
## Multi-dialecte (chantier portabilité — idempotency + session portés)
...
- **`colKit` (`entity/colKit.ts`, INTERNE — pas dans index.ts)** : `IFrameworkTableSpec` (kinds `text|json|bool|epochMs|int` + pk/notNull/unique/defaultFn/onUpdateFn + indexes) → `buildFrameworkTable(dialect, spec)` · `createFrameworkTableFactory(spec)` = `createXTable(dialect)` mémoïsée (1 instance/dialecte). Types par dialecte DANS le kit : epochMs=`integer` sqlite/`bigint mode:number` pg · json=`text mode:json`/`jsonb` · bool=`integer mode:boolean`/`boolean`. Index validés EAGER (fail-loud au build — le callback extraConfig drizzle est lazy). RÈGLE : toute entité à porter passe par le colKit ; mysql = étendre le kit seul. Défauts JS-level uniquement (`$defaultFn` — DDL dérivé n'émet pas DEFAULT).
- **Repository PK-portable** : `#pickOne(where)` = `pk IN (SELECT pk FROM (SELECT pk … LIMIT 1) AS picked)` — PK découverte lazy (`getTableColumns().filter(primary)` puis `getTableConfig().primaryKeys` composite), mémoïsée `#pk`. Table dérivée = MySQL (LIMIT-in-IN interdit + ERROR 1093) ; composite = row-values `(a,b) IN`. Fallback `rowid` UNIQUEMENT sans PK (sqlite-only). `#affected` normalise `changes` (better-sqlite3) / `rowCount` (pg) pour delete/updateMany. ⚠️ Resté dialecte : `limit(-1)` OFFSET-sans-LIMIT (sqlite-only, PG rejette) → router au portage typé ; upsert `onConflictDoUpdate` (mysql=`onDuplicateKeyUpdate`) ; `returning()` absent mysql.
- **Session portée** : `sessionEntity.ts` = spec + `createSessionTable(dialect)` (colKit) + `createSessionEntity`/`registerSessionEntity(orm, dialect)` dynamiques — **`@entity` SUPPRIMÉ** (imposait sqlite à l'import). Enregistrement = wire `registerStores.ts` (`SESSION_PORTED = sqlite+postgres`, factory no-op : le storage s'enregistre à l'import). `SESSION_ENTITY_NAME`. e2e PG `session-postgres.e2e.test.ts` gaté `NF_PG_URL` = **1ᵉʳ run du DrizzleRepository générique sur PG** (upsert/touch/gc/verbes bornés).
- **Dette** : `#tables`/`#db` typés SQLite ; en PG stockés via cast (runtime prouvé OK). `getRepository` PG typé = portage typé du repo (S3).
- **Reste (1-2 entités/session)** : user (⚠️ `findBySocialProvider` json_each → `@>` pg / `JSON_CONTAINS` mysql), token, webauthn, totp, webhook, audit ; puis mysql (`mysql2`) + DDL prod drizzle-kit.
...
- Tests (vitest) : `npm test` = **163** (18 fichiers ; 7 = e2e PG session, gatés `NF_PG_URL`, skip sans) — config Zod, banc orm-core, jointure complexe, SessionStorage IoC/CRUD, User P5.9, stores token/webauthn/totp/audit/webhook/idempotency, auto-register, **colKit (14)**, **repository `#pickOne` (9)**, **session PG e2e (7)**. Banc ADR-0002 User↔Room + age.
