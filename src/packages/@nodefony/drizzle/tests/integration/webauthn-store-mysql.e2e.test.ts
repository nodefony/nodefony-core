...
/**
 * Contrat `IWebAuthnCredentialStore` — dialecte **mysql** (MÊME suite que
 * sqlite/postgres). Chemins divergents traversés : `ON DUPLICATE KEY UPDATE` +
 * relecture (pas de RETURNING), `tinyint` pour les booléens (le contrat doit
 * rendre un VRAI booléen), JSON rendu en string par MariaDB (customType).
...
describe.skipIf(!MYSQL_URL)(
  "DrizzleWebAuthnCredentialStore — contrat (mysql)",
  () => {
