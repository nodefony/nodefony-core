...
import { SESSION_ORM, type SessionRow } from "../../nodefony/entity/sessionEntity";
...
describe("Drizzle SessionStorage — mécanisme IoC + CRUD (P7.4)", () => {
  ...
  describe("registre (IoC)", () => {
    it("le storage Drizzle s'auto-enregistre sous 'drizzle'", () => {
      assert.equal(SessionsService.getStorage("drizzle"), DrizzleSessionStorage);
    });
  ...
  describe("CRUD", () => {
    it("write puis read restitue Attributes/metaBag/flashBag/user", async () => {
      await storage.write(
        "sid1",
        { Attributes: { a: 1 }, metaBag: { m: 2 }, flashBag: { f: 3 }, user: "bob" },
        "default",
      );
      const r = (await storage.read("sid1", "default")) as Record<string, unknown>;
