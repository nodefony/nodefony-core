...
import { missingHistoryColumn } from "../../nodefony/src/migrator/history";
import { checkDataAdvice } from "../../nodefony/src/migrator/destructive";
...
describe("une table d'historique qui n'est pas la nôtre", () => {
  describe("missingHistoryColumn — trois moteurs, trois grammaires", () => {
    it("sqlite : le message exact qui a conduit à la destruction", () => {
      assert.equal(missingHistoryColumn("no such column: source"), "source");
    });

    it("postgres", () => {
      assert.equal(
        missingHistoryColumn(`column "run_id" does not exist`),
        "run_id",
      );
    });
    ...
    it("mysql", () => {
      assert.equal(
        missingHistoryColumn("Unknown column 'finished_at' in 'field list'"),
        "finished_at",
      );
    });
    ...
    it("🔴 une colonne APPLICATIVE ne déclenche RIEN — c'est un autre incident", () => {
      ...
      assert.equal(missingHistoryColumn("no such column: slug"), null);
      assert.equal(missingHistoryColumn(`column "title" does not exist`), null);
    });

    it("une panne qui ne parle pas de colonne reste au fourre-tout", () => {
      assert.equal(missingHistoryColumn("ECONNREFUSED 127.0.0.1:5432"), null);
      assert.equal(missingHistoryColumn("access denied for user"), null);
    });
  });

  describe("checkDataAdvice — la base est NOMMÉE, la copie est expliquée", () => {
    it("sqlite : le fichier à copier, et l'ordre de ne pas le recréer", () => {
      const p = checkDataAdvice("default", {
        dialect: "sqlite",
    ...
    it("porte toujours les deux interpréteurs et le connecteur visé", () => {
      const p = checkDataAdvice("facturation", { dialect: "sqlite" });
