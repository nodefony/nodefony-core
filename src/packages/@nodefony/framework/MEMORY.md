...
- **Backplane cross-process** (`IBackplane` = _fond de panier_ : tuyau qui relie les hubs des autres process ; `publish`=local+backplane, ingress backplane=`publishLocal` only, anti-boucle via `originId`) :
