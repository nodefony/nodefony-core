...
- [`../realtime/MEMORY.md`](../realtime/MEMORY.md) — « Socket Nodefony » (Hub, RealtimeController, backplane cluster/Redis, sonde) : module dédié, n'est PLUS dans framework
...
Module Nodefony : routeur HTTP+WS, Controller, Resolver, décorateurs `@route`/`@controller`/`@controllers` + NestJS-inspired HTTP method + response decorators, moteur de vues **Eta** (unique — remplace Twig/EJS, retirés).
...
| --- | --- | --- |
| `Controller` | 514 | Extends Service. Props: `route`, `request`, `response`, `context`, `queryGet/Post/File`, `body` (alias de `queryPost`). `session` = **getter** sur `context.session` (pas de `startSession()` ; activation = `@UseSession`/`@Session`/cookie). API: `renderJson/renderView/renderFileDownload/renderMediaStream` (moteur Eta — pas de `renderTwig`/`renderEjs`), `redirect`, `forward`, `getSession`, `getFlashBag/setFlashBag`. **⚠ noms d'action réservés (conflit prop/méthode Controller) : `session`, `get`, `set`, `request`, `response`, `method`, `body`.** |
...
| `Eta` | — | Service template **unique** (`"template"`, remplace Twig/EJS retirés). `render(str, params)` / `renderFile(path, params)` → Promise\<string\>. `cache` true en prod uniquement. |
...
## Admin data plane — `IAdminApi` / `AdminBroker`
...
- **RBAC fail-closed** (`adminRbac.ts` `isAdminGranted`) : un endpoint à rôle n'est accordé que si l'appelant le porte ; `roles=[]` → **403**. Le firewall (zone `nodefony-admin`) garantit l'authentification en amont.
...
- **kernel debug runtime** : `GET /nodefony/kernel/api/log/level` (état : `globalDebug` + `overrides` module→seuil + `expiresAt` countdown) + `PATCH` (`{module, level, ttlMs?}` set ; `level:"off"` clear). **Prod-safe** (≠ `config/{module}` dev-only — débugger un incident = prod) : RBAC `ROLE_NODEFONY_ADMIN`, par module **ou `*`** (debug tout — wildcard core, même TTL), niveau validé strict (`Syslog.severityFromInput` → 422), **TTL imposé** (défaut 15 min / plafond 60 min → jamais de debug oublié allumé), audité (catégorie `log`). Délègue au core `Syslog.setDebugOverride`/`clearDebugOverride`.
- **framework** : `GET /nodefony/framework/api/{routes,info,admin}` — `createFrameworkAdminApi(broker, opts?)`. `routes` = dump `Router.routes` ; `admin` = **catalogue discovery P10.2** (tous les producteurs + descriptors + endpoints, ordonné par `descriptor.order`) → source de la nav admin Studio. Le framework héberge le broker → register direct dans `onKernelReady` avant `mountAll`.
- **framework playground (DEV-ONLY)** : `GET /nodefony/framework/api/playground/routes` — `PlaygroundAdminApi.ts` (`buildPlaygroundSnapshot()`), greffé par `createFrameworkAdminApi(broker, { playground: true })` **uniquement si** `environment==="development" || debug` (le playground Studio EXÉCUTE des actions ; hors dev → 404, la page affiche « dev uniquement »). Snapshot form-ready groupé par controller : transports (`methods` + `duplex` = WEBSOCKET déclaré), params décorés (`paramsMeta` **triés par index** — les décorateurs s'évaluent droite→gauche), gardes (`security.clauses`, `scopes`, `idempotent`, CSRF, `sessionIntent`, `bypassFirewall`). Exclut `AdminApiController` (déjà catalogué par `admin`) et les routes sans controller. RBAC défaut broker (`ROLE_NODEFONY_ADMIN`). Consommé par la page Studio `/nodefony/playground` (formulaires auto — aucun code généré dans l'app). Tests : `tests/unit/PlaygroundAdminApi.test.ts`.
- **http** : `GET /nodefony/http/api/{servers,info}` (cf http MEMORY.md).
- **realtime** : `GET /nodefony/realtime/api/health` — le producteur (et tout le hub/sonde « Socket Nodefony ») vit dans le module **`@nodefony/realtime`**, auto-enregistré (cf [`../realtime/MEMORY.md`](../realtime/MEMORY.md)). Namespace `realtime` (pas `framework`).
...
- **kernel `config/schema`** : `GET /nodefony/kernel/api/config/schema[?module=<nom|basename>]` — le **CATALOGUE** des clés assignables, là où `config` rend l'**ÉTAT**. Tableau PLAT (`ISchemaCatalogRow`) : `key` (chemin pointé, EN PREMIER — le rendu tient la 1ʳᵉ colonne pour l'identité et ne l'élague jamais), `module`, `type` (les valeurs de l'`enum` si présent, sinon le type), `default`, `effective`, `source` (`default`/`app`/`env`/`runtime`, via `provenance`), `note` (`réservé`/`secret`/`dérivé du kernel`/`modifiable à chaud`, depuis les `.meta()`), `description` (le `.describe()` du schéma). Source = `flattenConfigSchema` (cœur, `config/schemaCatalog.ts`) × `buildConfigEntries()` — aucun module n'a rien à exposer en plus, le JSON Schema publié par `Module.configSchema()` suffit. Filtre inconnu ⇒ **404 qui NOMME** les modules acceptés (un catalogue vide se lirait « ce module n'a pas de config »). Effectif déjà REDACTÉ par `computeConfigEntry`. Porte CLI : `nodefony inspect schema <module>` (sujet `schema`, `filter: "module"`, `render: "cards"`).
...
**Runner tests framework** : **unit = vitest** (`vitest run`, `vitest.config.ts` ; mocha+chai tournent via `globals:true` + shim `import "mocha"` + alias stub mongoose). **coverage = `vitest run --coverage`** (v8, json-summary/lcov → `.coverage/`). **intégration = vitest** (`vitest.integration.config.ts`, tape le serveur). Pourquoi vitest et pas monocart : monocart `--require` bascule en CJS → specs important le dist non enregistrées (cf [[feedback_coverage_modules]]).
...
### `@Idempotent` (classe + méthode, dual — anti double-effet des mutations)
...
- **Clés d'idempotence CLOISONNÉES par application** (`RedisIdempotencyStore`, dernier param `resolvePrefix`) : `nf:<app>:idem` via `RedisService.keyPrefix("nf:idem")` (typage structurel + garde `typeof === "function"` → module Redis antérieur = préfixe historique, jamais un crash). POURQUOI plus grave qu'une fuite de lecture : l'idempotence REND la réponse mémorisée — deux apps partageant l'espace, la demande de l'une passait pour un rejeu de l'autre et recevait sa réponse. L'atténuation existante (clé déjà scopée `[identity, clientKey]` par `evaluateIdempotency`) reposait sur une convention d'APPELANT, pas sur le stockage : deux apps ont volontiers un compte `admin` et une clé cliente prévisible. Tests avec contrôle négatif (`RedisIdempotencyStore.test.ts`). Store drizzle : cloisonné par sa BASE, rien à faire.
...
// (liste complète des décorateurs + controllers auth = voir index.ts — source de vérité, ne pas dupliquer ici)
// decorators
export {
  route,
  controller,
...
- **Router Pass 2 (405 Allow) host-aware + AGRÉGÉE RFC 9110 §15.5.6** : une route
