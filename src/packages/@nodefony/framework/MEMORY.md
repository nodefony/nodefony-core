...
- **kernel debug runtime** : `GET /nodefony/kernel/api/log/level` (état : `globalDebug` + `overrides` module→seuil) + `PATCH` (`{module, level, ttlMs?}` set ; `level:"off"` clear). **Prod-safe** (≠ `config/{module}` dev-only — débugger un incident = prod) : RBAC `ROLE_NODEFONY_ADMIN`, **par module seul** (bascule globale = env `NF__DEBUG`), niveau validé strict (`Syslog.severityFromInput` → 422), **TTL imposé** (défaut 15 min / plafond 60 min → jamais de debug oublié allumé), audité (catégorie `log`). Délègue au core `Syslog.setDebugOverride`/`clearDebugOverride`.
