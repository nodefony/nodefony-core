...
- **+ endpoints paramétrés** (introspection cross-module, producteur **kernel**) :
  ...
  - `GET /nodefony/kernel/api/module/{name}/coverage` — dernier rapport de couverture (`<module>/.coverage/coverage-summary.json`, **vitest+v8**), `{available,total,files[],generated}`. Onglet Studio Coverage. `available:false` si pas généré.
