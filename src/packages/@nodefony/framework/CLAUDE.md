...
| Aspect | Fait |
| --- | --- |
| Types exports | condition `"nodefony-source": "./index.ts"` (la **source**, lue par les tsconfigs du dépôt via `customConditions`) devant `types` → `dist/types/` (lu par l'installeur) : évite la race TS2307 quand http type-check framework avant son build. **Retirer la condition ou le `customConditions` casse les consommateurs.** |
