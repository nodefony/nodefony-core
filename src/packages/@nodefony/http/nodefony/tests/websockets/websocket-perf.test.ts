...
describe("WEBSOCKETS PERF — Concurrent connections", function () {
  ...
  it("10 simultaneous connections open and close cleanly", (done) => {
    const N = 10;
  ...
  it("25 simultaneous connections — all receive handshake", (done) => {
    const N = 25;
    let handshakes = 0;
    let closed = 0;
    const sockets: WebSocket[] = [];
...
describe("WEBSOCKETS PERF — Message throughput", function () {
  ...
  it("100 sequential messages on one connection — all echoed", (done) => {
    const ws = openWs(`${WSS}/nodefony/test/ws/echo`);
    const N = 100;
    let received = 0;
  ...
  it("50 messages × 1 KB — all delivered", (done) => {
    const ws = openWs(`${WSS}/nodefony/test/ws/echo`);
    const N = 50;
...
describe("WEBSOCKETS PERF — Round-trip latency", function () {
  ...
  it("Single round-trip under 500ms", (done) => {
    const ws = openWs(`${WSS}/nodefony/test/ws/echo`);
    let handshakeDone = false;
    let t0: number;
  ...
  it("10 sequential round-trips — avg < 200ms", (done) => {
    const ws = openWs(`${WSS}/nodefony/test/ws/echo`);
    const N = 10;
    const rtts: number[] = [];
    let handshakeDone = false;
    ...
    const sendNext = () => {
      if (seq < N) {
        ...
        expect(avg).to.be.lessThan(200, `avg RTT ${avg.toFixed(1)}ms exceeded 200ms`);
        ws.close(1000);
      }
    };
...
describe("WEBSOCKETS PERF — Routing under load", function () {
  ...
  it("10 concurrent connections on route variables — all resolve correctly", (done) => {
    const N = 10;
    let correct = 0;
    let closed = 0;
    ...
    for (let i = 0; i < N; i++) {
      const id = `item${i}`;
      const ws = new WebSocket(`${WSS}/nodefony/test/ws/routes/${id}`, wsOpts);
      ws.on("error", done);
      ws.on("message", (data) => {
        try {
          const msg = JSON.parse(data.toString());
      ...
      ws.on("close", () => {
        closed++;
        if (closed === N) {
          expect(correct).to.equal(N, `only ${correct}/${N} resolved correctly`);
          done();
        }
      });
    }
  ...
  it("5 concurrent connections on 2-variable route — all resolve correctly", (done) => {
    const N = 5;
    let correct = 0;
    let closed = 0;
    ...
    for (let i = 0; i < N; i++) {
      ...
      ws.on("message", (data) => {
        try {
          const msg = JSON.parse(data.toString());
          if (msg.variables?.var1 === v1 && msg.variables?.var2 === v2) correct++;
          ws.close(1000);
        } catch {
          done(new Error("parse error"));
        }
      ...
      ws.on("close", () => {
        closed++;
        if (closed === N) {
          expect(correct).to.equal(N, `only ${correct}/${N} routes resolved correctly`);
          done();
        }
      });
    }
