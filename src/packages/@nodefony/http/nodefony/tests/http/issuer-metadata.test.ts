...
/**
 * Cette application PUBLIE-t-elle ses métadonnées — `null` si oui, la raison sinon.
 *
 * Le rôle émetteur n'est pas universel : il exige `security.jwt.issuer` en URL
 * https, et une application lancée en **production** sans `NF_JWT_ISSUER` n'en a
 * pas — délibérément, puisqu'elle ne peut pas deviner son adresse publique
 * derrière un relais (RFC 8414 §2). Ce banc n'a alors rien à éprouver.
 *
 * Il saute donc, mais **en le DISANT** : un fichier sauté en silence est un vert
 * qu'on croit — et c'est ce banc qui a fait rougir la marche `production` de la
 * forge, en accusant le code d'une capacité absente par conception.
 */
  ...
  try {
    ...
    if (res.status !== 200) {
      return (
        ...
        `métadonnées d'émetteur. Attendu en production sans NF_JWT_ISSUER.`
      );
    }
    const doc = JSON.parse(res.body) as { issuer?: unknown };
    return doc.issuer === ISSUER
      ? null
      : `émetteur publié « ${String(doc.issuer)} » ≠ « ${ISSUER} » — ce banc est ` +
          `écrit pour l'émetteur de développement.`;
  } catch (e) {
...
if (raisonDuSaut !== null) {
  ...
  process.stderr.write(
    `\n[issuer-metadata] SUITE SAUTÉE — le décor manque :\n  - ${raisonDuSaut}\n\n`,
  );
}

describe.skipIf(raisonDuSaut !== null)(
  "Émetteur — métadonnées RFC 8414",
  () => {
...
describe.skipIf(raisonDuSaut !== null)(
  "Émetteur — jeu de clés publiques",
  () => {
    ...
    it("⭐ la boucle se ferme : le document mène AU jeu de clés réellement servi", async () => {
      ...
      const doc = JSON.parse((await get(METADATA)).body) as {
        jwks_uri: string;
      };
