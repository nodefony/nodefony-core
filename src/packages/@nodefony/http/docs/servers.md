...
(`server-http.ts:83`) crée le `http.Server`, applique les réglages de transport, branche
...
`http-kernel.ts:562`, via `buildTrustProxy()`, `trustProxy.ts:100`) : aucune structure allouée par
