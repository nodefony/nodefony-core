...
- **`servers.portPolicy: "auto" | "strict"`** — que faire si le port désiré est occupé. Défaut **`auto` en `development`**, **`strict` en `production` ET `test`** (`resolvePortPolicy`, porté par le CŒUR et réexporté par `portBinder` — `doctor` juge un port tenu par la même règle). Prod : le port est un CONTRAT (service k8s/ingress/sonde) → glisser en silence = pod « sain » injoignable. Test : un port pris = un serveur resté debout → le banc doit s'arrêter, pas viser le serveur du voisin. `servers.portRetryAttempts` (défaut 20) borne le repli.
...
- **Validation au boot** : `index.ts` `onKernelRegister` → `defineHttpConfig(this.options, this.kernel)` → **ré-assigne `this.options`** (sûr : `onRegister` AVANT instanciation `@services` à `onBoot`). Lève une `BootConfigurationError` `[@nodefony/http] configuration invalide — chemin : message` (via `parseModuleConfig`) si invalide.
- **strict (refus) vs loose (passthrough)** — décision clé, et **jamais `z.object`** : sections transmises à une **lib tierce** (`http`/`https`/`http2`/`websocket(s)`/`queryString`/`statics.*.options`) = **`z.looseObject`** (sinon Zod stripperait une option lib légitime — ex. `http.insecureHTTPParser`). Sections **notre code** (`securityHeaders`/`certificates`/`session`/`upload`/`rateLimit` + la **RACINE**, qui porte `trustProxy`) = **`z.strictObject`** : une clé inconnue INTERROMPT le boot en la nommant. `z.object` RETIRAIT la clé en silence — `trustProxi` acceptée, défaut appliqué, personne prévenu. Le refus passe par `parseModuleConfig` (cœur) → `BootConfigurationError`, fatale en dev comme en prod ; une `Error` nue serait absorbée par le fail-soft.
- **Schéma PUR** (pas de deref kernel/env) → `defineHttpConfig` injecte les défauts kernel APRÈS parse : `upload.uploadDir` vide ← `kernel.tmpDir` (sinon `/tmp`) ; `certificates.selfSigned.attrs` vide ← `commonName=kernel.domain`.
- **Piège Zod 4** : `.default(() => sub.parse({}))` par section (un `.default({})` plat ne ré-applique PAS les sous-défauts).
- **Métadonnées de champ** (`.meta()` NATIF zod, typé `IConfigFieldMeta` du core via augmentation `GlobalMeta`) : flags `reserved`/`runtimeMutable`/`kernelDerived`/`secret` écrits dans le global registry Zod → recopiés par `httpConfigJsonSchema()` (`z.toJSONSchema`) pour Studio/doc. Posés : `http3`=reserved, `headerServer`=runtimeMutable, `uploadDir`+`selfSigned.attrs`=kernelDerived. ⚠️ poser le `.meta()` sur le **nœud final** présent dans `.shape` (sinon non lu : http3 a fallu wrapper le `.default()` racine).
...
- Tests : `tests/unit/httpConfig.test.ts` (25, vitest). Exports publics : `httpConfigSchema`, `defineHttpConfig`, `httpConfigJsonSchema`, `meta`, types `IHttpConfig`/`IHttpConfigInput` (préfixe `I` — un seul nom par type, republié par `nodefony/interfaces/IHttpConfig.ts` ; jamais d'alias, qui en publierait deux).
...
**Probes santé `/livez` + `/readyz` (config `health`, défaut on)** : court-circuit TOTAL dans `HttpKernel.onHttpRequest` — AVANT le rate-limit (un kubelet throttlé 429 = cascade de restarts), 0 contexte/session/ALS/log par sonde, réponses pré-allouées (`HEALTH_BODY_*`), match STRICT de `request.url` (avec query → pipeline normal). `livez` = 200 tant que le process sert, Y COMPRIS pendant le drain (sinon k8s tue le pod en plein graceful). `readyz` = 200 si `kernel.postReady && !#terminating && kernel.readinessBlocked === 0` ; 503 pendant boot, dès SIGTERM, et tant qu'un composant RETIENT la mise en service. Ce troisième terme est le **registre de disponibilité** du core (`kernel.setReadiness(nom, prêt, raison?)`, `readinessRegistry.ts`) : un module qui a un état d'amorçage — schéma de base en retard, cache froid — pose un verdict DÉJÀ calculé, la sonde ne fait que lire un entier (0 allocation, 0 `await`, registre `null` tant que personne ne s'inscrit). Une sonde qui interrogerait une base tomberait avec elle ; celle-ci non, et le pod redevient disponible SEUL dès que le verdict repasse à vrai — sans redéploiement. Le journal du pod NOMME ce qui retient (CRITIC à la bascule) ; le corps de la sonde reste la constante pré-allouée. `health.shutdownDelay` (défaut 0) = délai additionnel entre la bascule 503 et le drain (propagation LB ; alternative : `preStop: sleep N` k8s). Chemins servis par HTTP ET HTTPS (kubelet `scheme: HTTPS` ok).
...
- **Porte SOCKET — l'unité profilée est l'INVOCATION, pas la connexion** (`src/profiler/FrameProfile.ts`). Un `WebsocketContext` vit pour la connexion : ses `phases` (tableau + index par NOM) et son `requestId` ne peuvent pas décrire une frame parmi N — y empiler les phases donnait une timeline **cumulative** ET une croissance sans borne (le Resolver y poussait `initialize` à CHAQUE frame du pont). `WebsocketContext.beginFrame(method, url)` ouvre un `FrameProfile` (id `<requestId de la connexion>.<n° de frame>`, compteur `#frameSeq`), `collectFrame(fp)` le passe au Profiler. **`FrameProfile` satisfait structurellement ce que `collect()` lit d'un contexte** → aucune modification du Profiler (`kind: "ws"` sortait déjà de `ctx.type`). `null` si ni profiler ni timing (prod) = 0 alloc.
- **Redirection des phases** : `WebsocketContext` **override** `phaseStart`/`phaseEnd` → si une invocation est ouverte (lue dans l'ALS `RequestContext.payload.invocation`), la phase va dans SON tableau ; sinon (handshake) comportement de base. **Le chemin HTTP n'est pas touché** (0 risque de régression sur le hot path). L'index d'une frame est un **scan arrière**, pas une `Map<nom,idx>` → une phase ré-entrante est supportée (la table par nom écraserait la 1ʳᵉ occurrence).
- Buffer ORM par frame : le kernel refusait de l'allouer au handshake (« cumulatif sur N messages, donc mensonger ») — **par invocation il redevient exact** (`FrameProfile.profilerQueries` passé en `queries` de l'ALS par le pont). Mesuré : la porte socket ne paie ni `parse`, ni `firewall`, ni `send` (donc pas `saveSession`), mais paie une phase **`identity`** (re-validation `token.isValid()` = lecture du store de session, à CHAQUE frame).
...
- **`body` = alias de `queryPost`**, UN seul sens dans tout le framework (celui d'`IAdminRequest` et de l'écosystème) : le CORPS, jamais la query string. 3 chemins, même valeur — `controller.body`, `request.body` (getter, `Request.ts:112`), `request.request.body` (posé sur la requête Node à `fireRequestEnd`). ⚠️ Ne PAS confondre avec `query`, qui FUSIONNE GET+POST (`Request.ts:313`). Banc : `decorators.test.ts` « body-alias ».
...
- **Sessions admin** : `GET sessions/list` (pagination **SERVEUR** `?user&limit&offset`, défaut 50/cap 200 ; `total` absent + `nextCursor` présent sur backend curseur) · `POST sessions/{ref}/revoke` · `POST sessions/revoke-user/{identifier}` (logout-everywhere) · `GET sessions/mine` (self-service). RBAC `ROLE_NODEFONY_ADMIN` (défaut broker). **DTO redacté `ISessionSummary`** (jamais `Attributes`/`flashBag`/id brut) — id remplacé par **`ref = HMAC(secret, id)`** (`computeSessionRef`/`toSessionSummary`, fonctions pures testées ; secret = `this.secret` session). Champ **`current`** = la session qui émet la requête (`currentSessionRef()` lit le contexte de l'ALS, 1× par page) : seul moyen de DÉSIGNER une ligne — le client ne sait pas calculer le HMAC de son cookie, et dans `mine` toutes les lignes ont le même `user`. Mutations auditées via pont optionnel `auditService` (catégorie `session`, no-op si security absent).
- **Pagination NATIVE des sessions** (le standard `IPage`/`IPageQuery` du core) : `ISessionStorage.listPage?(ISessionListQuery)` + `countSessions?()` — **c'est `listPage` qui fait foi** pour `supportsEnumeration()` (absent → **501** honnête). Filtres portables `user` + `authenticated` (anonyme = `user` vide ; stocké `NULL` en SQL/Mongo → `{$null:…}`). Ordre contractuel `updatedAt DESC`, départagé par id. **Redaction par construction garantie sur `listPage`** (`Attributes`/`flashBag` toujours vides — SQL ne les SELECTe pas, mémoire/Redis les vident) ; `listAll?()` reste le dump cold-path, libre de les porter. Modes : **offset** (Drizzle via `paginate()` orm-core, Mongoose, mémoire → `total` exact) · **curseur** (Redis SCAN → `nextCursor`, pas de `total`, `countSessions` = **-1** = « inconnu » assumé).
- **Compteurs de tête** : `GET sessions/stats` → `{total, authenticated, anonymous, users}`, chacun `number | null`. **Endpoint SÉPARÉ de `sessions/list`** : ces nombres ne dépendent ni de la fenêtre ni de l'ordre, les rejouer à chaque tour de page coûterait 3 `COUNT` pour un résultat identique. Mêmes filtres que la liste (`SESSION_FILTERS`), et il **refuse** ce que personne ne lit ; les clés de fenêtre (`limit`/`offset`/`order`) sont admises et **sans effet** (le client envoie ici le même query string qu'à la liste). Service : `SessionsService.countSessionFacets(filtres)` = `countFacets(SESSION_FACETS, …)` (cœur) + `countDistinctUsers`. **`null` = le backend ne sait pas** (`-1` du store traduit) → la console affiche « — », jamais `0` (un zéro se lit comme une absence). Front : `fmtFacet` (UI kit `components/ui/pageQuery.ts`). 🔴 Un endpoint de compteurs **NE FILTRE PAS** la dimension qu'il décompose (`facetDimensions`, cœur) — ici le trou ne peut pas s'ouvrir, `SESSION_FILTERS` ne porte que `user` là où les facettes décomposent `authenticated` ; un test le verrouille.
- **`countDistinctUsers?()`** — « 400 sessions » n'est pas « 400 personnes ». Ce n'est **pas** un `COUNT` filtré mais une agrégation ⇒ **capacité de BACKEND** (déclarée par store, Redis rend `-1`), pas une facette du contrat. S'appuie sur `IRepository.countDistinct(champ, critère)` (orm-core) : `COUNT(DISTINCT col)` SQL / `$group`+`$count` Mongo / `Set` en mémoire. `NULL` non compté (l'anonyme ne fait pas un utilisateur). ⚠️ **Le décorateur `RevocationGuardStorage` doit le relayer** — une capacité perdue dans un décorateur affiche « inconnu » là où la base sait répondre (même piège que `sortableFields`).
- 🔴 **`user` et `authenticated` portent la MÊME colonne** (`authenticated` ≡ « `user` non nul ») : un critère AND-only n'en porte qu'une condition. Les stores SQL/Mongo **jetaient `authenticated`** quand `user` était fourni, là où le store mémoire appliquait les deux → même question, deux réponses selon le backend, et `?user=alice&authenticated=false` rendait les sessions d'alice comme « anonymes ». Contradiction ⇒ **ensemble vide** : `#criteria` rend `null`, l'appelant court-circuite sa requête. Garde dans le banc de contrat partagé.
- **Les compteurs prennent le FILTRE, pas la fenêtre** : `countSessions`/`countDistinctUsers` reçoivent `Partial<ISessionListQuery>` — un `COUNT` n'a pas de `limit`, et l'exiger obligeait l'appelant à inventer une valeur ignorée.
  ⚠️ **`SCAN COUNT` n'est pas un plafond** (Redis peut rendre plus de clés que demandé) → curseur **composite** `"<skip>:<curseurRedis>"` : on tronque à `limit` et on reprend dans le même batch. Même correctif appliqué à `RedisTokenStore` (bug identique, latent).
- **Parcours admin bornés** : `destroyByRef`/`destroyOwnByRef`/`destroyByUser` itèrent par pages de `SCAN_PAGE` (200) via `eachSessionRecord` — plus jamais de `listAll()` global. `destroyByUser` repasse **jusqu'à ce qu'un passage complet ne détruise rien** (`MAX_LOGOUT_PASSES`) : détruire pendant un parcours décale offset ET curseur faible → un seul passage en laisserait vivantes (prouvé par test). Méthodes `private` (soft TS) et non `#` : les tests d'orchestration instancient par `Object.create(prototype)`, qu'un brand check runtime rejetterait.
- **Bancs de contrat** (chez http, propriétaire du contrat, importés cross-package par les adapters) : `tests/support/sessionPaginationContract.ts` (partition sans trou ni doublon, borne `limit`, redaction, filtres, 2 modes) + `tests/support/sessionStoreContract.ts` (round-trip, `createdAt` insert-only, destroy idempotent, touch, gc par capacité `applicative`/`native-ttl`). Déroulés sur **6 backends réels** : mémoire + décorateur révocation (`tests/unit/session-pagination.test.ts`), Drizzle sqlite/PG/MariaDB/MySQL (greffé dans `session-store-contract.ts`), Mongoose, Redis (`session-store.test.ts` + TTL natif). Parcours façade multi-pages : `tests/unit/sessions-admin-paging.test.ts` (parc 450 > 1 page, 2 modes).
- Lecture défensive des services `server-{http,https,websocket,websocket-secure,static}` via `module.get(name)`.
- **Per-instance** : answers du process qui reçoit (LB route vers 1 pod). Header `x-nodefony-instance` posé par `AdminApiController` (convention `NF_INSTANCE_ID ?? pid`). Vue cluster = Redis P13. Cf [[project_multiprocess_scaling]].
