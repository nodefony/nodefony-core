...
### Phase 5c — Fuites mémoire ✅ partiel (2026-05-15)

Route `/nodefony/test/memory` ajoutée dans `DefaultController` → retourne `process.memoryUsage()` du serveur.
`nodefony/tests/http/memory.test.ts` créé — 8 tests (HTTP + WS). À valider avec serveur actif.

**Principe** : mesurer `heapUsed` du serveur via la route `/memory` avant/après charge.

**Tests créés** :
1. 1000 GET séquentiels → delta < 20 MB
2. 100 crashes sync → delta < 10 MB
3. 100 crashes async → delta < 10 MB
4. 100 crashes TypeError → delta < 10 MB
5. 500 requêtes mixtes (index + context + session) → delta < 20 MB
6. Serveur vivant après charge → /index 200
7. 100 WS open/close → delta < 10 MB
8. 50 WS echo round-trips → delta < 10 MB

**Note** : mesure server-side via route (pas `--expose-gc` côté test — server est un process séparé).
**Seuils généreux** (20 MB / 10 MB) pour absorber le bruit V8 GC — détecte les fuites catastrophiques.
...
### Phase 10 — Service Certificate — tests unitaires (pas urgent)

`nodefony/service/certificates.ts` — `node-forge` (RSA keypair + self-signed cert).

**Tests à créer** : `nodefony/tests/unit/certificate.test.ts`

| Test | Méthode | Assertion |
|---|---|---|
| Génération serial | `Certificate.generateSerial()` | entier positif, < 2^34 |
| Génération clés | `generateKeys()` | `keysPair.privateKey` + `.publicKey` non null |
| Création certificat | `createCertificate()` | `validity.notAfter` = `notBefore + 1 an` |
| Signature | `sign()` → `generateCertPem()` | PEM valide (commence par `-----BEGIN CERTIFICATE`) |
| Private key PEM | `generatePrivateKeyPem()` | commence par `-----BEGIN RSA PRIVATE KEY` |
| Public key PEM | `generatePublickeyPem()` | commence par `-----BEGIN PUBLIC KEY` |
| Fullchain | `createFullChain()` | contient cert PEM |
| Write/Read fichiers | `writeCertificates()` → `readCertificates()` | fichiers sur disque lisibles + contenus identiques |
| Force regen | `generateServerCertificates(true)` | nouveaux fichiers différents |
| Check existence | `checkCertificates()` → false si pas de fichiers | retourne false |

**Améliorations possibles** (service) :
- Bug subtil `createFullChain()` : opérateur `+` sur `||` → concaténation incorrecte (ligne 166-170)
- `serialNumber: "01"` hardcodé dans `createCertificate()` (ligne 267) ignore `generateSerial()`
- Typo : `serverParh` (devrait être `serverPath`) ligne 57
- `readCerticates` typo (devrait être `readCertificates`) — méthode publique
