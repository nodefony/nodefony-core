...
1. **Court-circuit à coût nul** — `endpointCount()` (`webhooks.ts:534`) lit la taille d'une `Map` :
...
- **`reveal` est un `POST`, pas un `GET`** (`WebhookAdminApi.ts:493`) : un secret n'a rien à faire
