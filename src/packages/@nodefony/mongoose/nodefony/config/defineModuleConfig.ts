 ...
 *
 * - `MONGODB_URI`   → `uri` du connecteur primaire (`nodefony`, sinon le premier).
 *   ⚠️ La place pour le secret de connexion (user:pass) — JAMAIS dans le dépôt.
...
function applyEnvOverrides(config: IMongooseConfig): IMongooseConfig {
  ...
  if (env.MONGODB_URI) {
    ...
    if (target && config.connectors[target]) {
      config.connectors[target].uri = env.MONGODB_URI;
    }
  }
