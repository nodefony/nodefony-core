...
// Serveur Mongo PARTAGÉ (globalSetup, ReplSet — les transactions l'exigent),
...
describe.skipIf(!URI)(
  ...
  () => {
    ...
    beforeAll(async () => {
      orm = new MongooseOrm("mongo_adv_a", URI!);
      await orm.connect();
    ...
    it("updateMany : modifie TOUTES les lignes du critère et retourne le count", async () => {
      ...
      const n = await users.updateMany(
        { age: { $gte: 30 } },
        { active: false },
      );
...
describe.skipIf(!URI)(
  "Mongoose avancé — eager-load many-to-one / one-to-one",
  () => {
    let orm: MongooseOrm;
    ...
    beforeAll(async () => {
      orm = new MongooseOrm("mongo_adv_b", URI!);
      await orm.connect();
...
describe.skipIf(!URI)(
  "Mongoose avancé — API ITransaction (idempotence bas niveau)",
  () => {
    let orm: MongooseOrm;
    beforeAll(async () => {
      orm = new MongooseOrm("mongo_adv_tx", URI!);
      await orm.connect();
    });
    afterAll(async () => {
      await orm.disconnect();
...
describe.skipIf(!URI)(
  "Mongoose avancé — many-to-many = garde-fou (non portable)",
  () => {
    it("une relation many-to-many fait ÉCHOUER la connexion (→ getNativeConnection)", async () => {
      ...
      const orm = new MongooseOrm(ORM_C, URI!);
      try {
