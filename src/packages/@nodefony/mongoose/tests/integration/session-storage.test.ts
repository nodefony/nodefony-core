 ...
 */
const EXTERNAL_URI = process.env.MONGO_TEST_URI;

describe("Mongoose SessionStorage — IoC + CRUD + sondes (Ph.2)", () => {
  ...
  describe("sondes data plane", () => {
    it("describeConnection : driver mongodb + cible SANS credentials + version", () => {
      ...
      assert.ok(!info.target.includes("@"), "target ne doit pas fuiter d'auth");
