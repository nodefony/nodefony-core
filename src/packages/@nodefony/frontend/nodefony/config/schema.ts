...
/**
 * Schéma Zod de la configuration de `@nodefony/frontend`.
 *
 * **Source de vérité** : ce schéma. `config.ts` en dérive les défauts
 * (`frontendConfigSchema.parse({})`), `defineFrontendConfig()` valide la fusion
 * `défauts + module.options` au boot, et `frontendConfigJsonSchema()` en produit
 * le JSON Schema introspectable consommé par le panneau de config Studio.
 *
 * ⚠️ NE PAS éditer les défauts ailleurs : modifier les `.default(...)` ICI.
 *
