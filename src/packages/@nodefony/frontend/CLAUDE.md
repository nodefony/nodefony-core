...
## Mode prod (`nodefony frontend:build`)

- Appelle `vite.build()` PROGRAMMATIQUEMENT (in-proc, OK car build est one-shot)
- Pas de superviseur ; Vite tourne dans le process Node et se termine
- Génère `manifest.json` dans `outDir` — lu par `TemplateHelper.renderProdTags()` (TODO)
