...
- `remoteDevListenAdvice(detection, listenHost)` (pur) : plateforme distante détectée + `devHost` de boucle locale ⇒ WARNING au `startDev` — le port existe et n'est joignable que depuis la machine qui sert (symptôme : page chargée, 0 script, `ERR_CONNECTION_REFUSED`). **On AVERTIT, on n'ajuste PAS** : élargir l'écoute est un geste réseau, jamais silencieux. Le message nomme les DEUX causes — `devHost` d'abord, puis `http.trustedHosts` (même écran vide une fois l'écoute élargie). Appelé hors de `resolvePublicOrigin`, qui retourne avant la détection quand `publicOrigin` est écrite.
...
- **Dérivation par requête** : `renderTags/renderDocument(entry, nonce?, requestHost?)` → `FrontendService.derivableHost()` décide, `originWithHostname(origin, host)` (pur, `remoteDev.ts`) exécute. Remplace le NOM seul — scheme+port restent ceux de Vite (page http:5151 → assets https:5173). 3 gardes : épinglage (`originPinnedBy` posé au `startDev`) · `HttpKernel.isTrustedHostname(h)` (règle UNIQUE, résolue par nom) · `trustedHosts !== true` (bypass = CSP ne couvre que loopback+domain). L'épinglage a DEUX sources d'autorité inégale : `"config"` (`publicOrigin` écrite) bloque toute dérivation, boucle locale comprise ; `"platform"` (Codespaces/Gitpod déduits) CÈDE devant `isLoopbackHostname(h)` — un client venu d'un tunnel local est servi en local, et `TemplateHelper` RECOMPOSE alors `scheme://host:status.port` au lieu de substituer le nom (une origine de forwarder n'a pas de port explicite : substituer rendrait `https://localhost`, soit le port 443). Motif : l'origine publique exige la session de la plateforme, qu'un client non humain (CI, sonde, agent) n'a pas. ⚠️ `hmr.host` reste figé au démarrage sur le template de la plateforme : les assets d'un client loopback chargent, son socket HMR non. Host inexploitable → `null` → origine résolue. PROD : ignoré (manifest = chemins relatifs). Helpers Eta (`Controller.withFrontendLocals`) propagent nonce+domain automatiquement.
...
- **CSP posée AVANT le 1er spawn Vite** : `startDev` déclare la PLAGE planifiée (`familyPortBlocks` = bloc de chaque famille, port-retry compris) puis rejoue `#registerCsp()` après `ready`. Motif : `startDev` part sur `onServersReady` — les serveurs écoutent DÉJÀ, et un CSP est FIGÉ pour la durée de la page (jamais renégocié) ⇒ une page servie pendant le démarrage de Vite perdait son HMR jusqu'au rechargement dur. Ports = `FrontendService.cspPorts()` : bloc entier tant que la famille ne SERT pas, port réel dès `state ready|compiling`. ⚠️ `status().port` vaut `null` tant qu'aucun port n'est RÉSOLU (conforme à `IViteSupervisorStatus`) — il ne retombe plus sur le port espéré. On contrôle néanmoins l'ÉTAT ici : un port résolu par une famille qui n'est plus prête ne sert pas. Le resserrement n'est pas cosmétique : 3 familles × 4 hôtes = en-tête CSP de ~7,9 Ko/réponse contre 2,3 Ko une fois les ports connus (limite courante des relais : 8 Ko). Jamais de `ws:`/`wss:` sans hôte — ce serait supprimer la garantie, pas le symptôme.
