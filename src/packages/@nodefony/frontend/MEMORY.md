...
- Priorité origine : `frontend.publicOrigin` config (validée, invalide = ERROR + dérivation locale) > détection plateforme > dérivation `browserReachableHost(devHost)`:port (0.0.0.0/:: → 127.0.0.1). Env app : `NF_FRONTEND_PUBLIC_ORIGIN`.
...
- `viteAllowedHosts()` : `trustedHosts` http (`true`→`true` ; strings converties ; RegExp ignorées WARNING) + domaine kernel + hôte du template — 1 liste (ouvrir un hôte dans trustedHosts ouvre 421 ET Vite). Vite accepte IP/localhost d'office (source Vite 8 vérifiée : 403 sinon, prouvé au banc témoin).
