...
| Sondes `nodefony-browser` | `skills/nodefony-browser/scripts/`      | `inspect.mjs` (mesure : contraste WCAG calculé, tailles, scripts servis, capture) · `watch.mjs` (frames WS, réponses ≥ 400) · `lib/browser.mjs` (lancement, connexion, ouverture garantie). Exécutés DANS le conteneur `<app>-browser` — chemins POSIX, aucun problème de plateforme |
