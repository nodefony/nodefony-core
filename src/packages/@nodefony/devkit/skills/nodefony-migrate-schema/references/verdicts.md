...
| `NF_MIGRATE_SECONDARY_CONNECTOR` | écrire une migration pour un connecteur qui n'est pas `default` | rien : seul `default` possède `migrations/` ; le schéma d'un secondaire se dérive (`ddl: "auto"`) ou se gère hors du framework |
