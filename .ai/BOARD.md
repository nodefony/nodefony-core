...
> Empreinte prise le **2026-09-13 11:47** (UTC).
...
| **10.0.0-beta** | ![10.0.0-beta](https://img.shields.io/github/milestones/progress-percent/nodefony/nodefony-core/7?style=flat-square&label=) `█░░░░░░░░░` 14% | 5 | 32 | 2026-10-14 |
...
Ordre 9.91 · P1 — figé à la création · 5.5 j · jalon 10.0.0-alpha · frise 2026-09-15 → 09-15
...
| --- | --- | ---: | --- | --- | --- |
| 1.5 | P1 — figé à la création | 0.5 | 2026-10-05 → 10-05 | #255 | test(release): contrôler les README publiés avant chaque publication |
| 1.6 | P1 — figé à la création | 1 | 2026-10-06 → 10-06 | #293 | build(release): cesser de réécrire les types au moment du publish |
| 1.7 | P2 — décision | 0.5 | 2026-10-07 → 10-07 | #275 | fix(release): écarter du changelog les commits qui n'atteignent aucun installeur |
...
| 3 | P2 — décision | 0.5 | 2026-10-12 → 10-12 | #259 | ci(release): publier l'image sur le registre de GitHub, sans aucun secret |
| 4 | P1 — figé à la création | 2 | 2026-10-13 → 10-13 | #155 | docs(agents): rendre le dépôt lisible par un agent web |
| 4.2 | P1 — figé à la création | 0.5 | 2026-10-14 → 10-14 | #158 | docs(site): publier llms.txt, le plan du site et robots.txt |
| 4.3 | P2 — décision | 1 | 2026-10-15 → 10-15 | #159 | docs(api): publier une référence d'API générée par paquet |
| 4.4 | P1 — figé à la création | 0.5 | 2026-10-16 → 10-16 | #160 | chore(github): poser les gabarits de ticket et de fusion |
| 4.5 | P1 — figé à la création | 0.5 | 2026-10-19 → 10-19 | #215 | fix(build): réparer le contrôle de format du code généré |
| 4.6 | P2 — décision | 0.5 | 2026-10-20 → 10-20 | #104 | test(cli): un seul processus pour vérifier la forme du code généré |
| 4.8 | P1 — figé à la création | 2 | 2026-10-21 → 10-21 | #20 | test(security): attaquer les paquets publiés avant leur sortie |
| 4.9 | P1 — figé à la création | 2 | 2026-10-22 → 10-22 | #294 | test(cli): éprouver l'installation d'une app avec pnpm, yarn et bun |
| 5 | P1 — figé à la création | 3.5 | 2026-10-23 → 10-23 | #268 | feat(security): rendre Keycloak utilisable de bout en bout pour la connexion externe |
| 5.1 | P0 — bloque le reste | 2 | 2026-10-26 → 10-26 | #269 | test(security): éprouver la connexion OpenID Connect contre un vrai Keycloak |
...
| 5.3 | P2 — décision | 0.5 | 2026-10-28 → 10-28 | #272 | docs(security): documenter le branchement d'un Keycloak, du realm au premier login |
| 9.3 | P2 — décision | 1 | 2026-10-29 → 10-29 | #359 | docs(corpus): recaler les 433 ancres fichier:ligne qui ont dérivé |
| 9.4 | P2 — décision | 0.5 | 2026-10-30 → 10-30 | #213 | build(scripts): typechecker les outils du dépôt, aujourd'hui hors de tout tsconfig |
| 9.5 | P2 — décision | 0.5 | 2026-11-02 → 11-02 | #176 | fix(orm): ne plus voir une destruction dans une table sqlite reconstruite |
| 9.6 | P1 — figé à la création | 0.5 | 2026-11-03 → 11-03 | #313 | fix(cli): resservir la vraie erreur de démarrage, pas celle de la dernière tentative |
| 9.7 | P1 — figé à la création | 1 | 2026-11-04 → 11-04 | #314 | fix(orm): ne pas replier sur un dialecte que les entités ne parlent pas |
| 9.8 | P2 — décision | 0.5 | 2026-11-05 → 11-05 | #352 | fix(http): empêcher un frontal de retenir un flux d'événements |
| 11 | P2 — décision | 0.5 | 2026-11-06 → 11-06 | #139 | fix(security): ne plus laisser de sessions et jetons sans propriétaire |
| 29 | P2 — décision | 0.5 | 2026-11-09 → 11-09 | #62 | fix(cli): sonder les ports réellement utilisés par l'application |
| 29.5 | P1 — figé à la création | 1 | 2026-11-10 → 11-10 | #381 | fix(http): ne pas laisser pendre une requête quand la session échoue |
| 30 | P2 — décision | 0.5 | 2026-11-11 → 11-11 | #25 | ci(tests): remettre au vert le test de tenue dans la durée |
| 32 | P2 — décision | 1 | 2026-11-12 → 11-12 | #80 | chore(pilotage): confronter au code les cases de la feuille de route |
| 46.1 | P1 — figé à la création | 0.5 | 2026-11-13 → 11-13 | #371 | test(devkit-bench): constater qu'un agent écrit hors de son décor |
| 46.2 | P0 — bloque le reste | 0.5 | 2026-11-16 → 11-16 | #372 | fix(devkit-bench): chercher la zone de firewall où elle vit vraiment |
...
| Ordre | Prio | Jours | Frise | Ticket | Titre |
| --- | --- | ---: | --- | --- | --- |
| 9.91 | P1 — figé à la création | 5.5 | 2026-09-15 → 09-15 | #316 | feat(scaffold): rendre l'application générée déployable en production |
| 9.94 | P0 — bloque le reste | 2 | 2026-09-16 → 09-16 | #369 | chore(release): publier la préversion 10.0.0-alpha.6 |
| 9.95 | P3 — fin de cycle | 0.5 | 2026-09-17 → 09-17 | #382 | refactor(scaffold): faire rédiger le message du semis par le framework |
| 9.955 | P1 — figé à la création | 1 | 2026-09-18 → 09-18 | #358 | feat(cli): donner à une application un contrôle de sa propre image |
| 9.96 | P2 — décision | 1.5 | 2026-09-21 → 09-21 | #321 | feat(scaffold): rendre l'application déployable en Kubernetes |
| 9.97 | P1 — figé à la création | 2 | 2026-09-22 → 09-22 | #322 | feat(ci): faire prouver par la vitrine ce que nul n'éprouve |
...
| 20 | P2 — décision | 0.5 | 2026-09-29 → 09-29 | #292 | chore(env): trancher le préfixe des variables propres à l'application |
| 26 | P1 — figé à la création | 0.5 | 2026-09-30 → 09-30 | #99 | feat(devkit): apprendre à l'agent à migrer un schéma |
| 28 | P1 — figé à la création | 0.5 | 2026-10-01 → 10-01 | #154 | fix(frontend): stabiliser le décalage de port sur les agents macOS |
| 55 | P3 — fin de cycle | 3 | 2026-10-02 → 10-02 | #30 | feat(mongoose): compléter les stockages manquants côté MongoDB |

## Jalon 10.1.0 — 29 ouverts

| Ordre | Prio | Jours | Frise | Ticket | Titre |
| --- | --- | ---: | --- | --- | --- |
| 12 | P2 — décision | 5 | 2026-11-24 → 11-28 | #378 | feat(core): rendre traduisibles les textes du framework |
...
| Ordre | Prio | Jours | Frise | Ticket | Titre |
| --- | --- | ---: | --- | --- | --- |
| 12 | P2 — décision | 8 | — | #379 | feat(core): servir une application dans la langue du visiteur |
...
| Ordre | Prio | Jours | Frise | Ticket | Titre |
| --- | --- | ---: | --- | --- | --- |
| 59 | P1 — figé à la création | 0.5 | 2026-11-18 → 11-18 | #47 | docs(release): faire suivre à l'accueil ce qui est réellement publié |
| 60 | P3 — fin de cycle | 1.5 | 2026-11-19 → 11-19 | #27 | chore(release): publier les paquets de la version 10 sur npm |
