...
auth de base (Anonymous/UserPassword/Jwt/OAuth2 client interne/mTLS/APIKeys) **+ à intégrer P6 : Passkeys/WebAuthn (FIDO2), Token Exchange RFC 8693 (délégation agents), DPoP, Argon2id, OAuth 2.1+PKCE** ·
...
━━━━━━ NODEFONY · MIGRATION ━━━━━━━━━━━━━━━━━━━ vérifié code 2026-08-20 ━━━━━━
 ...
 P11 CLI par module        █████░░░░░  50%   3✅  3🔶  2⬜   ◀ BLOQUEUR MVP — lifecycle + scaffold ✅ ; orm:migrate ✅ (S5c soldé, #98 : cinq verbes × 3 dialectes, chaque réglage sur son couple) ; reste user:* métier
 P12 Couche IA agentic     █▍░░░░░░░░  14%   0✅  2🔶  5⬜   🧪 différé (llm réel non intégré ; protocole MCP AU CŒUR, module vide ; agent-guard vide)
 P13 Realtime distribué    ████████▌░  84%  11✅  2🔶  2⬜   (reste Kafka 13.6a/b · décorateurs 13.8)
 ...
 P16 Cloud-Native (10 axes)██████▌░░░  63%  20✅  0🔶 11⬜ ⏭️2  (33 sous-items · 16.G.2/G.3/G.4 fermés par les guides publiés · 16.H.6 : le banc existe · 16.F.3 CADUC · reste = THÈME du jalon 10.1)
────────────────────────────────────────────────────────────────────────
 GLOBAL                    ███████▉░░  79% 155✅ 19🔶 37⬜  (211 tâches · somme des lignes ci-dessus · recalé le 09-19 après confrontation des 52 cases au code — #80)
────────────────────────────────────────────────────────────────────────
...
> et porte son verdict, son ancre dans `src/` et sa destination. La règle tenue : **une partie
> livrée ne coche pas la case** — un travail à moitié fait reste 🔶, jamais ✅. Les deux passes
> précédentes avaient échoué exactement là-dessus.
>
> ⚠️ **Ce que cette passe ne dit pas** : elle a lu du code, elle n'a rien exécuté. Une case peut
> décrire un symbole qui existe et ne marche pas. Et ces états restent posés à la MAIN : ils
> revieilliront. C'est pourquoi le fichier est destiné à être démonté —
> [#384](https://github.com/nodefony/nodefony-core/issues/384).
...
| # | Tâche | État |
| --- | --- | --- |
| 🔶 P3.6 | Filtrage par requestId (CLI) | `Pdu.requestId` livré (`src/nodefony/src/syslog/Pdu.ts:197`) et le filtre existe déjà en fonction pure (`syslog/drivers/filterPdus.ts`) ; reste **uniquement l'enveloppe CLI** (LB.3b) — aucune commande de namespace `logs:`/`syslog:` n'est enregistrée. Confronté au code le 09-19. |
| 🔶 P3.8 | Rate limit logs par requestId | anti-flood livré mais **global par instance** (`src/nodefony/src/syslog/Syslog.ts:1215` — compteur `start`/`burstPrinted` porté par le Syslog, pas par une clé) ; reste la clé par requestId. Confronté au code le 09-19. |
| ⏭️ P3.10 | Transport NCSA/Combined dédié | absorbé par P3.11 (driver `file`) |
| 🔶 P3.11 | **Log Backplane** (write↔read pluggable) | LB.W+LB.0→LB.5+LB.4 ✅ ; défaut `queryDriver:auto` cluster-aware + garde-fou k8s multi-pod (`83f8b9ff`/`a0408f10`) ; reste LB.3b CLI — **même travail que P3.6 et P11.5**, un seul ticket les soldera. Confronté au code le 09-19. |
...
| # | Tâche | État |
| --- | --- | --- |
| 🔶 P5.0 | Gate batch/console (`initServers` par type) | **Porté par [#233](https://github.com/nodefony/nodefony-core/issues/233)** (jalon `10.2`). `isConsole()` + dispatch 0-serveur OK ; reste `BatchCommand` — 0 occurrence du symbole. Confronté au code le 09-19. |
| ⬜ P5.0b | Service Cron/Worker (worker dédié) | **Porté par [#233](https://github.com/nodefony/nodefony-core/issues/233)** (jalon `10.2`), avec P5.0 — même ticket, même geste. Décision : gardé, découplé serveur. Rien de commencé : le seul ordonnanceur du cœur est `GcScheduler`. Confronté au code le 09-19. |
| ⏭️ P5.7 | ~~Adapter Sequelize User~~ | **caduc (virage ORM : sequelize supprimé)** |
| 🔶 P5.10 | Tests User cross-ORM | couvert **de facto** par 2 bancs miroirs même contrat `IUserRepository` (Drizzle 8 + Mongoose 8) ; banc paramétré unifié = **optionnel, et c'est un choix, pas une dette** — les deux bancs existent (`mongoose/tests/integration/user-mongoose.test.ts`, `drizzle/tests/integration/user-champs-metier.test.ts`), aucun banc unifié n'est requis pour que le contrat soit couvert. **Cette case n'appellera pas de ticket.** Confronté au code le 09-19. |
| ✅ P5.12 | `Redis` SessionStorage | **LIVRÉ, enregistrement automatique COMPRIS** — la case annonçait « reste l'enregistrement automatique au boot », c'était FAUX : `redis/nodefony/src/SessionStorage.ts:323` appelle `SessionsService.registerStorage("redis", RedisSessionStorage)` au CHARGEMENT du module, exactement comme `files`, `drizzle` et `mongoose` ; `redis/index.ts:92` documente que l'export suffit à l'exécuter. Confronté au code le 09-19. |
...
| --- | --- | --- |
| 🔶 P6.8 | `authorization.ts` (3 niveaux) | Niveau B partiellement porté par [#71](https://github.com/nodefony/nodefony-core/issues/71) (hiérarchie de rôles extensible, jalon `10.2`) — l'ACL fine par ressource reste sans ticket — **et n'en aura pas avant que P17 multi-tenant la redéfinisse** (`project_multitenant_chantier_kit` pose le RBAC tenant-aware comme préalable) ; recherche d'un contrôle d'accès par ressource dans `@nodefony/security` : zéro, vérifié le 09-19. J6 `4e336d50` : RoleVoter + `decide()` affirmative+DENY véto, voterRegistry ; reste niveau B (ACL fine par ressource) |
| ⬜ P6.9d | **Serveur d'autorisation OAuth 2.1** (code flow + PKCE) | **Porté par [#244](https://github.com/nodefony/nodefony-core/issues/244)** (jalon `11`), avec le mTLS de P6.9b. **NON FAIT — TRANCHÉ (décision user) : APRÈS la release 10.0.0.** Conséquence assumée : la 10.0.0 sort avec un MCP dont la sécurité est complète et prouvée, mais dont le jeton s'obtient **hors bande**. Le report déplace la charge sur la DOCUMENTATION (comment obtenir et poser le porteur). Nodefony ÉMET des jetons et publie de quoi les vérifier (RFC 8414 + JWKS), mais n'offre aucun **flux d'obtention** normatif : ni `authorization_endpoint`, ni `token_endpoint`. Conséquence CONSTATÉE sur un vrai client : le SDK MCP exige les deux champs et refuse la connexion — alors que le document est LÉGAL sans eux (RFC 8414 §2). **Ce n'est PAS un prérequis du MCP authentifié** : la spec impose le porteur, pas le moyen de l'obtenir. C'en est un pour un client qui doit obtenir son jeton SEUL. Recontrôlé le 09-19 : ni `authorization_endpoint` ni `token_endpoint` côté serveur. |
| ⬜ P6.9b | `MTlsAuthenticator` | **Porté par [#244](https://github.com/nodefony/nodefony-core/issues/244)** (jalon `11`), avec P6.9d — un seul ticket tranche les deux. Zones admin (niche, non démarré) : 0 occurrence du symbole. Confronté au code le 09-19. |
| ✅ P6.10 | Logs auth + CSP stricte + headers | **LIVRÉ, logs auth COMPRIS** — la case annonçait « reste logs auth dédiés », c'était FAUX : `security/nodefony/service/auditService.ts:37` porte un journal dédié (« login, refus d'accès, jeton émis/révoqué, défense CSRF/CORS, verrou WS »), alimenté par `src/audit/recordAudit.ts` et émis par les **cinq** authenticators (`ApiKey`, `ExternalJwt`, `Jwt`, `Session`, `UserPassword`) ainsi que par le firewall. J5-A `SecurityHeaders` (CSP + Referrer + COOP/COEP/CORP + Permissions-Policy). Confronté au code le 09-19. |
| 🔶 P6.15 | Studio — section Sécurité | Pages LIVRÉES : Sessions/Users/API Keys/Firewall/Audit/Profil self+admin (`8d54bf4a`→`486eb75b`) ; reste le live `security:audit` — **porté par [#236](https://github.com/nodefony/nodefony-core/issues/236)** (jalon `10.2`, avec la vue Services de P10.10). Aucun canal de ce nom n'est déclaré. Confronté au code le 09-19. |
| ⬜ P6.16 | WebAuthn — `rpId` dynamique par Host (multi-vhost) | `#rpID` toujours figé au boot — `security/nodefony/service/webAuthn.ts:102` (`#rpID = "localhost"`), résolu une fois en `:134-140` depuis la config → dériver du `Host` de la requête. Confronté au code le 09-19. |
...
| # | Tâche | État |
| --- | --- | --- |
| 🔶 P8.4 | `Metrics` runtime | **Porté par [#228](https://github.com/nodefony/nodefony-core/issues/228)** (jalon `10.2`). via Studio (ticker stats → canal `nodefony:supervision`, `studio/nodefony/realtime/providers.ts`), pas service standalone ; `/metrics` Prometheus = **16.J, même ticket #228** — 0 occurrence de `prom-client` ni de la route, vérifié le 09-19 |
...
| # | Tâche | État |
| --- | --- | --- |
| ✅ P9.2 | Barrels `index.ts` | **CONSTATÉ le 2026-09-06**, pas déduit. Tout module LIVRÉ porte son barrel : les seuls `index.ts` manquants sont ceux des modules `agent`, `agent-guard`, `llm`, `mcp`, `memory`, `rag`, `vector` — **la couche IA de P12, différée et vide par construction** — plus des gabarits (`templates/`) et des artefacts de couverture (`.coverage/`), qui ne sont pas du code. Le cœur `src/nodefony` n'a pas d'`index.ts` racine et n'en a pas besoin : son entrée est `src/index.ts`, et `exports["."]` pointe le `dist` bâti depuis là. |
...
| # | Tâche | État |
| --- | --- | --- |
| 🔶 P10.6 | Auth admin (`ROLE_NODEFONY_ADMIN`) | **Porté par [#33](https://github.com/nodefony/nodefony-core/issues/33)** (jalon `10.0.0`). rôle actif sur **5 routes** — `StudioController.ts:205` et les 4 de `StudioCreateController.ts:66-152` (`@IsGranted`) ; reste : le généraliser aux **12 autres** routes de la surface `/nodefony` (SPA, health/info, realtime-info). Confronté au code le 09-19. |
...
| 🔶 P10.10 | Vues services/profiling | **Profiling ✅** (`studio/frontend/src/routes/logs/ProfilingTab.tsx`) · **Services = ébauche** (`studio/frontend/src/routes/stubs.tsx`). Le reste n'est donc pas « incrémental » comme l'annonçait cette case : c'est UNE vue à écrire, l'autre est livrée. **Portée par [#236](https://github.com/nodefony/nodefony-core/issues/236)** (jalon `10.2`), avec le live d'audit de P6.15. Confronté au code le 09-19. |
| ⬜ P10.11 | Tests intégration studio | **13 fichiers de test unitaires** sous `studio/nodefony/tests/unit/` — l'annonce « 0 test studio » (06-28) est périmée. Ce qui manque reste vrai : **aucun test de bout en bout** (`studio/nodefony/tests/integration/` est vide). Le back est couvert e2e via security. Confronté au code le 09-19. |
...
| # | Tâche | État |
| --- | --- | --- |
| 🔶 P11.1 | Tests intégration commandes existantes | filet spawn livré (`NF_RUN_CLI_BOOT=1`) + **renforcé `0b883854`** (typo=EX_USAGE 64, happy-path commande de module `http:network` 1-Kernel/0-serveur, point d'arrêt `onReady`, `status` standalone 0-Kernel) ; **refacto parse-pur `4a7d46ae`** (resolveCommand point unique, `runProfile` déclaratif → commande de module SERVER possible, `finishOrPark` uniforme) ; le filet couvre aujourd'hui `src/nodefony/src/tests/CliIntegration.test.ts`, `create.test.ts` et les e2e `orm:*` de drizzle ; **lancement détaché natif `be9ca8b2`** (`--detach --wait --health`, fast-path standalone, `start.sh` = wrapper mince) ; reste les commandes métier. Confronté au code le 09-19. |
| 🔶 P11.2 | Commandes `http:*` | **DEUX existent** — `http:certificates` (`http/nodefony/command/certificatesCommand.ts:25`) et `http:network` (`networkCommand.ts:20`) : la case ⬜ était donc fausse. Ce qui reste n'a jamais été énoncé ; la case ne cite aucun verbe manquant. **À trancher : nommer les commandes attendues, ou clore.** Confronté au code le 09-19. |
| 🔶 P11.3 | Commandes `framework:*`/`security:*`/`user:*` | **`security:secrets` ✅ (`ac095e38`→`faa743eb`)** : génère les clés (TOTP/webhook/CSRF, 32 o base64) + câblage 3 fichiers `.env.local`→`env.ts`→`use()`, détection par clé, `--write` gardé (refus si tracké git) — référencée par les 3 warnings « clé ÉPHÉMÈRE » ; **`security:user:add` ✅ (`4ac7245e`)** : création de compte (hash Argon2id via UserService, prompt masqué TTY, `--admin` = rôles Studio), fix kernel `finishOrPark` console à `onPostReady` (process fantôme). Le module porte en réalité **six** commandes (`security-secrets`, `security-token`, `security-user-add`, `security-user-delete`, `security-user-list`, `security-user-password`) : `security:user:password` **EXISTE**, la case le donnait à tort comme restant. Restent `framework:*` (**aucun** répertoire `command/` dans le module) et `user:*` métier (idem). Confronté au code le 09-19. |
...
| ⬜ P11.5 | Commandes `logs:tail/filter` + bridge Studio | LB.3b — **même travail que P3.6 et P3.11** : une seule enveloppe CLI à écrire au-dessus de `filterPdus`, comptée trois fois dans la carte. Aucune commande de namespace `logs:` ou `syslog:` n'existe. **Verdict : un seul ticket pour les trois, à ouvrir quand LB.3b sera pris.** Confronté au code le 09-19. |

### P12 — Couche IA agentic (14 %) — 🎯 **JALON 12 (MAJEURE STABLE)** · cf skill `nodefony-roadmap`

> **Rangée le 2026-09-06 dans le jalon `12`.** C'est un étage au-dessus de la plateforme : il
> suppose le socle stable, et il n'est pas sur le chemin de la 10.
...
| # | Tâche | État |
| --- | --- | --- |
| 🔶 P12.1 | `@nodefony/llm` | module réel (**8** src hors tests, tests **vitest**, script `test` présent) — **non intégré au kernel** : le seul consommateur est `@nodefony/memory`. Compté le 09-19. |
| ⬜ P12.2 | `@nodefony/vector` | **5** src, tests `bun:test` inertes (aucun script `test`). Compté le 09-19. |
| ⬜ P12.3 | `@nodefony/rag` | **6** src, tests `bun:test` inertes. Compté le 09-19. |
| ⬜ P12.4 | `@nodefony/memory` | **5** src, tests `bun:test` inertes. Compté le 09-19. |
| ⬜ P12.5 | `@nodefony/agent` | **4** src, tests `bun:test` inertes. Compté le 09-19. |
| 🔶 P12.6 | `@nodefony/mcp` | module vide (0 src, pas de `package.json`) MAIS protocole/gardes/catalogue déjà AU CŒUR — **5 fichiers** sous `src/nodefony/src/mcp/` — + porte éprouvée (devkit lot 7, P6.9.1→9.4). Compté le 09-19. |
| ⬜ P12.7 | `@nodefony/agent-guard` | répertoire ABSENT (0 src, pas de `package.json`). Constaté le 09-19. |
...
| # | Tâche | État |
| --- | --- | --- |
| 🔶 P13.1 | TCP/UDP/Unix sockets | scaffold seul — **aucun serveur TCP, UDP ni socket Unix n'est implémenté** (recherche `server-tcp`/`server-udp`/`UnixSocket` dans `http` et `realtime` : zéro). **Verdict : n'aura PAS de ticket** — niche différable dont personne n'a exprimé le besoin ; elle se rouvrira quand un usage la réclamera. Confronté au code le 09-19. |
| ✅ P13.2 | `@nodefony/redis` refactor | **Même tâche que P7.7** (doublon de carte, constaté le 09-19). Fondation conventions faite ; **cloison des clés par application** `e43530ae`+`2820a219` (`redis/nodefony/src/keyNamespace.ts` — sessions, jetons, WebAuthn, idempotence) ; ~110 tests. Aucun reste énoncé : la case se clôt, cf P7.7. |
| ⬜ P13.6a | `KafkaBackplane` (driver seul) | **Porté par [#243](https://github.com/nodefony/nodefony-core/issues/243)** (jalon `11`), avec P13.6b. 4 briques réutilisées telles quelles ; seam transport injectable, 0 dep ajoutée. Le nom reste INTERDIT tant que le driver n'existe pas — `realtime/nodefony/tests/unit/backplaneRegistry.test.ts:36` le garde. Confronté au code le 09-19. |
...
| # | Tâche | État |
| --- | --- | --- |
| 🔶 P14.7 | CLI `frontend:create/build/dev` | **`frontend:build`, `frontend:dev` et `frontend:status` livrés** (`frontend/nodefony/command/`) ; **`frontend:create` n'existe pas** — `frontend/nodefony/command/` ne contient que `frontend-build.ts`, `frontend-dev.ts` et `frontend-status.ts` ; le scaffold passe par `nodefony create module --frontend <fw>`. Reste : trancher si `frontend:create` doit exister, ou retirer ce nom de la case. Confronté au code le 09-19. |
| ⬜ P14.12 | Plugin Vite Nodefony (alias + env) | Voisin de [#131](https://github.com/nodefony/nodefony-core/issues/131) (composer les plugins Vite en un seul endroit, jalon `10.2`) — à trancher : même chantier, ou ticket propre. Aucun plugin Vite maison n'existe à ce jour (0 occurrence d'un `name: "nodefony…"` de plugin dans `@nodefony/frontend`). Confronté au code le 09-19. |
...
### P15 — Mediasoup + SIP/Asterisk (0 %) — 🎯 **JALON 11 (MAJEURE STABLE)**

> **Rangé le 2026-09-06 dans le jalon `11`**, avec P17 multi-tenant : deux extensions de
> PLATEFORME, additives, qui ne sont sur le chemin ni de la 10 ni l'une de l'autre.
...
| --- | --- |
| 16.A Kernel/Lifecycle | ✅ A.1 graceful drain COMPLET (`b94af814`→`d494730d` : http-terminator ×3, `shutdownTimeout`, WS 1001 avant drain, deadline `shutdownDeadline`, preuve `docker stop` → 200 + exit 0) · ⬜ A.2 événements `onPreShutdown`/`onDrain` (0 occurrence — recontrôlé le 09-19 ; le seul `onDrain` du dépôt est une variable locale de contre-pression de flux, `http/.../context/http/Response.ts:546`) · ⬜ A.3 bootstrap secrets avant DI (dépend 16.C, donc rien ne peut exister tant que C n'existe pas — vérifié le 09-19) · ⬜ A.4 warning PID 1 sans tini (0 occurrence dans le code — recontrôlé le 09-19 : `tini` n'apparaît que dans `docs/guides/docker-cloud-native.md` et le corpus de `nodefony-devops`) |
...
| 16.C Secrets | ⬜ C.1 `ISecretProvider` · ⬜ C.2 `SecretManager` · ⬜ C.3 hook boot · ⬜ C.4 migration des secrets — 0 occurrence des symboles `ISecretProvider`/`SecretManager` — recontrôlé le 09-19 ; dépend P6, design au kit |
| 16.D Docker | ⬜ D.1 `Dockerfile.dev` (aucun fichier de ce nom ; le nom n'apparaît que dans `docs/release/nodefony-10.md` — vérifié le 09-19) · ✅ D.2 Dockerfile PROD multi-stage (gabarit `create app` : HEALTHCHECK `/readyz`, USER 1000:1000, node PID 1, `mkdir` propriétaire de tmp/var pour qu'un volume nommé hérite des droits, `npm ci` si verrou, code laissé à root, étiquettes OCI, base `node:24-alpine` choisie sur mesures — 0 vulnérabilité critique contre 2 et 4 hautes contre 14 face à `node:24-slim`, pour 81 Mo de moins ; les candidates, leurs chiffres et les lignes à remplacer sont écrits en tête du gabarit, dans le README de l'app et sur la page publique de l'image — prouvé sur app générée depuis le tarball, `33c1f5c5`/`141201ca`) · ✅ D.3 compose (`docker/docker-compose.yml`) · ✅ D.4 profils (postgres/mongo/redis/kafka/tools/loki/opensearch/proxy/browser) · ✅ D.5 réseau bridge + alias DNS · ✅ D.6 topologie de PRODUCTION au gabarit (profil `edge` : l'app en image SANS port publié + un frontal nginx dont la configuration est DÉRIVÉE de l'application à la construction — `proxy:generate` sait terminer le TLS et lire l'arbre d'`assets:publish` ; aucune clé privée dans l'image, elle se monte. Prouvé sur app générée derrière un frontal réel : `Host` étranger 421, socket silencieuse vivante à 65 s, XFF forgé jeté, suite e2e rejouée à travers le proxy. `52a88140`/`36050510`) |
| 16.E Skills/Tooling | ⏭️ **E.1 `docker-debug` CADUC sous ce nom** — le besoin est couvert par `nodefony-devops` (§5 « le décor local » : capacités CONSTATÉES, cluster jetable `kind` ; §6 pièges) et par son renvoi explicite « un symptôme runtime à diagnostiquer → `nodefony-debug` » (`.claude/skills/nodefony-devops/SKILL.md:31,102`) · ✅ **E.2 `infra-up` FAIT, ailleurs qu'en skill** — `npm run test:all` démarre les conteneurs manquants et attend leur santé : `vitest.gates.ts:210` compose `docker compose … up -d <service>`, `scripts/test-all.ts:198` la lance · ⬜ E.3 détection conteneur dans start-server — toujours absente (`isContainer` de `src/nodefony/src/Tools.ts:222` est une garde de type sur le **Container DI**, faux ami ; ni `/.dockerenv` ni `/proc/1/cgroup` ne sont lus). Confronté au code le 09-19. |
...
| 16.J Métriques | ⬜ `/metrics` Prometheus — **porté par [#228](https://github.com/nodefony/nodefony-core/issues/228)** (jalon `10.2`), avec P8.4. 0 occurrence de `prom-client` ni de la route, recontrôlé le 09-19 ; repoussé, décidé 06-15 ; données déjà dispo `dashboard:stats` |

---

### P17 — Multi-tenant / SaaS (0 %) — 🎯 **JALON 11 (MAJEURE STABLE)** · POST cloud-native (P16)

> **Rangé le 2026-09-06 dans le jalon `11`**, avec P15. Le motif : c'est une couche **additive** sur
> des fondations déjà saines (ALS, Repository, firewall central, `withTransaction` comme patron de
> `withTenant`) — elle ne dépend pas de la couche IA, et la placer en `12` la mettrait derrière un
> chantier qui n'a pas commencé.
