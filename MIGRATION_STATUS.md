...
| 🔴 **Le RSS croît HORS de V8 sous charge soutenue** — sous ~10 000 rps, le RSS monte de **+33 MB/h** (R² 0,91 ; dernier quart **+46,5**) pendant que `heapTotal` (−4,8) et `external` (−5,4) **DESCENDENT** : ni arènes V8, ni tampons rattachés. **Aucun plateau** sur 75 min de charge cumulée, mesuré DEUX fois. Projection ~790 Mo/jour, **~2,2 Go sur 3 jours de pod**. ⚠️ **Le gate mémoire ne peut PAS le voir** et il a raison d'être vert : il mesure `heapUsed` sur quelques secondes, et le tas ne bouge pas. Pistes : fragmentation de l'allocateur système, piles de threads, allocations natives non déclarées à V8. Plan de traque en 5 étapes (la 1ʳᵉ = reproduire sous **Linux**, la fragmentation étant très dépendante de la plateforme) | 🔴 | mesuré 2026-08-25 sur macOS (`soak.mjs`, production, `--expose-gc`) ; données brutes `tmp/ventilation-rss-2026-08-25.csv` · mémoire IA `project_rss_leak_hunt_kit` |
...
━━━━━━ NODEFONY · MIGRATION ━━━━━━━━━━━━━━━━━━━ vérifié code 2026-08-20 ━━━━━━
 ...
 P8  CLI + Monitoring      █████████░  90%   4✅  1🔶  0⬜   (doctor ✅ ; chaîne de publication ✅ P8.5 — reste R6 : gestes GitHub hors dépôt + 1ʳᵉ publication manuelle)
