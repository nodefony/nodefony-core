...
| 🔶 P5.4 | Tests unit orm-core (registry, entity, decorators) + integration adapter réel | 7.5       | 1 ses.  | P5.3        | 🔶 2026-05-21 — **1er adapter réel** `SequelizeOrm` (`@nodefony/sequelize/nodefony/src/orm-core/`, distinct du legacy) validé sur sqlite `::memory:` (entités ADR-0002 User↔Room UUID). **6 tests verts** : @entity→compile models→repository, CRUD portable, relation, tx commit/rollback. **Verdict ADR-0003** : CRUD ✅ portable. **Fuites traitées (même session)** : eager-load assos déclarées → portable (`{relations}`), repo tx-aware (`withTransaction`), criteria `Partial<T>` type-checké, FK explicite. **orm-core 22 + Sequelize 7 verts.** **Reste** : valider le contrat enrichi sur un 2ᵉ adapter hétérogène (Mongoose : `populate` + session) avant P7.x ; opérateurs riches criteria → Drizzle P7.4 |
