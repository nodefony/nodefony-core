...
| 🔶 P13.4  | 🆕 `IRealtimeHub` interface + `LocalRealtimeHub` (dev) + `RealtimeService` central (façade, normalisation, dédup échos cluster)                                 | 3 ses. | P1.4 (ALS)         | 🔶 **SOCLE LIVRÉ 2026-05-24** — contrat **`IRealtimeSocket`** (core, ex-`IRealtimeHub` ; vocab figé socket=la prise / hub=broker) + **`RealtimeHub`** broker serveur (framework : canaux **PARTAGÉS**/fan-out, per-instance = `LocalRealtimeHub`) + `RealtimeClient implements IRealtimeSocket` (isomorphe) + **full-duplex entrant gated**. Commits 712013c→858e746. Reste : façade `RealtimeService` consommateur serveur + dédup cross-pod (backplane Redis) |
...
| 🔶 P13.7  | 🆕 Protocole **JSON-RPC 2.0 maison** + RPC bidirectionnel (Promise) + HTTP long-polling fallback + types partagés `ServerToClientEvents`/`ClientToServerEvents` | 4 ses. | P13.4, P1.7        | 🔶 **PROTOCOLE EXTRAIT 2026-05-23** — **`JsonRpcPeer`** (core, isomorphe) + base **`RealtimeController`** (framework) + **RPC bidirectionnel** (request/response, `kernel:ping`/`kernel:gc`) + seam **`IRealtimeTransport`** (`BrowserWsTransport`/`WsConnectionTransport`). Reste : HTTP long-polling fallback + types partagés `ServerToClientEvents`/`ClientToServerEvents`                                                                                  |
| P13.8     | 🆕 Décorateurs `@RealtimeController(path)` + `@RealtimeEvent(name)` (lecture `Reflect.metadata`)                                                                | 2 ses. | P13.7              | ⬜ base `RealtimeController` (classe) existe ; reste les **décorateurs** `@RealtimeController(path)`/`@RealtimeEvent(name)`. Pattern @route mais pour events temps réel                                                                                                                                                                                                                                                                                         |
...
| ⬜ P13.11 | 🆕 **Sonde « socket Nodefony »** (auto-observabilité realtime — patron sondes+hub)                                                                              | 1 ses. | P13.4              | ⬜ `probe()` sur `RealtimeHub` (canaux/abonnés/fan-out/full-duplex/backplane) + `buildRealtimeHealth()` + endpoint `/nodefony/realtime/api/health` + canal `realtime:health` + panneau Hub. Vue client (`getChannelStats`) déjà là. Mémoire `project_realtime_socket_probe` (idée user 2026-05-24)                                                                                                                                                              |
