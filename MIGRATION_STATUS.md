...
| 🔶 P6.18 | Audit — store PERSISTANT (dette de P6.14)                   | `audit.driver` pluggable via `auditStoreRegistry` (builtin `memory`) + **`DrizzleAuditStore`** append-only (query builder dialect-agnostique : pagination curseur exacte `(ts,id)`, gc rétention, dégradation gracieuse) — sémantique SQLite 7/7. Reste : adapters mongoose/redis + preuve pg cross-pod (slice multi-dialecte P7) |
