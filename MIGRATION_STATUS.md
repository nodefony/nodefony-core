...
Le framework lit **122** noms de variables ; **90** portent `NF_`. La famille `NODEFONY_*` est
**SOLDÉE** (0 restante) ; il reste **32** noms sans préfixe, dont une majorité qu'on ne possède pas.
...
| Génériques revendiqués par d'autres outils : `REDIS_HOST/PORT/PASSWORD/URL/TEST_URL`, `MONGO_TEST_URI`, `MONGODB_DEBUG`, `BCRYPT_COST/N`, `POD_NAME`, `MODE_START`, `NO_TTY` | 12 | renommer en `NF_*` — **priorité** : une collision ne lève aucune erreur, elle produit un comportement inexplicable |
| Interrupteurs de coût des bancs : `RUN_PERF`, `RUN_CLI_BOOT`, `RUN_CLUSTER_E2E`, `RUN_WS_RUPTURE`, `WS_RUPTURE_CAP/STEP` | 6 | renommer en `NF_*` — la règle du `CLAUDE.md` les vise nommément ; `NF_RUN_PERF` est déjà cité en doc sans être lu nulle part |
| Conventions d'écosystème qu'on LIT sans les posséder : `APP_ENV` (12-factor/Symfony), `DATABASE_URL`, `MONGODB_URI`, `PORT` (PaaS) | 4 | **ne pas renommer** — les lire est le contrat ; le Kernel fait déjà `APP_ENV \|\| NF_ENV` |
| Standards tiers : `NODE_ENV`, `NODE_DEBUG`, `NODE_NO_WARNINGS`, `UV_THREADPOOL_SIZE`, `BABEL_ENV`, `SHELL` | 6 | **ne pas toucher** — on ne les possède pas |
