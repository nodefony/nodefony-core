...
| 🔴 **Le RSS croît HORS de V8 sous charge soutenue** — sous ~11 000 rps, le RSS monte pendant que `heapTotal` et `external` restent plats ou **DESCENDENT** : ni arènes V8, ni tampons rattachés. Sur le run le plus long (88 min, 180/180 fenêtres), le RSS prend **+31,3 MB dont +31,1 hors V8 (99,4 %)** — tas réservé +0,2, externe 0,0. 🔴 **La grandeur qui TIENT d'un décor à l'autre est +0,55 MB par million de requêtes** (0,56 et 0,551 sur deux décors et deux débits distincts, 2 % d'écart) ; les pentes horaires vont de 18 à 46 MB/h sur les MÊMES runs — elles décrivent le poste, pas le code, et ne se citent pas nues. **Aucun plateau** sur aucun des quatre runs : le plus long fléchit sans s'aplatir (18,25 MB/h global, 13,83 sur la seconde moitié). Projection ~790 Mo/jour, **~2,2 Go sur 3 jours de pod**. ⚠️ **Le gate mémoire ne peut PAS le voir** et il a raison d'être vert : il mesure `heapUsed` sur quelques secondes, et le tas ne bouge pas. Pistes : fragmentation de l'allocateur système, piles de threads, allocations natives non déclarées à V8. Plan de traque en 5 étapes (la 1ʳᵉ = reproduire sous **Linux**, la fragmentation étant très dépendante de la plateforme) | 🔴 | mesuré 4× sur macOS (`soak.mjs`, production, `--expose-gc`) ; ⚠️ le banc ACQUITTAIT à tort un run tronqué (`verdict: "clean"`, exit 0 sur 17 % du run demandé) — corrigé `59a150a1` : `verdict: "incomplet"` + exit 2. Mémoire IA `project_rss_leak_hunt_kit` |
