...
| 🚨 P0.6 | **BUG-004** — leak scope DI sur WS **avec session** fermé au handshake       | Mémoire    | 1 ses.  | —           | OUVERT — pré-existant. `onFinish` rate `once("onSaveSession")` (event déjà émis au save handshake) → scope retenu. 100 WS open/close /ws (startSession) → 100 scopes. Sujet cycle de vie session (P2.x). Détails BUG_REPORT. |
