...
| 🔶 P10.3  | `IAdminApi` dans http, framework, syslog                                    | 2 ses.  | P10.2              | 🔶 2026-05-20 — **kernel ✅** (`/nodefony/kernel/api/{health,info,modules}`) + **http ✅** (`/nodefony/http/api/{servers,info}`, registre `onKernelBoot` sans importer framework) + **framework ✅** (`/nodefony/framework/api/{routes,info}` — dump des 125 routes via `createFrameworkAdminApi`). Header `x-nodefony-instance` per-instance. Validé runtime. **Reste : syslog** |
