...
| 🔴 **`NF__HTTP__TRUSTPROXY=1` fait ÉCHOUER le boot** — constat de l'audit 07-14 INSTRUIT et CONFIRMÉ 08-20 : `coerceEnvValue` rend `Number(1)` pour `"1"` (`envOverride.ts:51`) et le schéma `trustProxy` est `z.union([boolean, string, array])` — aucun membre n'accepte un number (`http config.ts:980`). Une variable d'env DOCUMENTÉE casse le boot : contraire à l'ADR-0006 (12-factor). Fix simple (coercition ciblée ou union élargie) + test | 🔴 | à corriger AVANT release |
...
>
> - **5 CONFIRMÉS → montés au tableau des dettes ci-dessus** : `NF__HTTP__TRUSTPROXY=1` casse le
>   boot (🔴) · `ClusterManager` arrache les handlers tiers · table de routes `static` globale ·
