...
- `[1× — 2026-07-28c]` ⭐ **Exiger « ce que tu n'as PAS vérifié » dans le rapport d'un sous-agent est ce qui a fait trouver le vrai défaut.** L'agent `sonnet` a livré `create service` avec ses preuves, puis listé de lui-même ses trous — dont « pas de smoke-test runtime ». C'est en comblant CE trou (booter l'app, lister les services) que le gabarit s'est révélé porter une méthode morte (`initialize` au lieu d'`init`). Un rapport lisse aurait clos le sujet sur du vert. La section est bon marché à demander et rentable à lire : **la déléguer, c'est déléguer la vigilance, pas seulement le travail.**
...
- `[2× — 2026-07-28]` 🔴 **Le DÉCOR décide de ce que le banc mesure — et un banc de découvrabilité lié au checkout mesure un agent MIEUX SERVI que l'utilisateur.** Preuve au transcript : un agent a lu `/…/src/nodefony/src/Service.ts` **en chemin absolu**. Corrigé comme pour le banc de schéma, mais surtout **avec la même implémentation** (`scripts/lib/isolation.mjs` partagé) : la recopier ferait diverger deux idées d'« isolé ». Effet immédiat, dès le premier run isolé : l'agent qui écrivait `@injectable` **ne l'écrit plus** — il le tenait de nos sources, et `@injectable` n'apparaît **nulle part** dans une app fraîche.
- `[1× — 2026-07-28]` **Le décor s'ENREGISTRE dans le rapport** (champ `decor`), au même titre que le modèle : deux runs de décors différents ne se comparent pas.
...
## ⚖️ Un JUGE faux est plus dangereux qu'un juge sévère

- `[4× — 2026-07-28]` 🔴 **Une sonde de banc qui recale un agent AYANT FAIT JUSTE est le mode de défaillance n° 1 — quatrième occurrence.** Premier run des 3 tâches de socle : **3 des 5 rouges étaient faux** (une sonde lisant les tests alors qu'elle visait la production ; une regex `[^)]+` bloquée par la parenthèse d'un appel imbriqué dans la charge loggée ; un interdit `setTimeout` posé sans la condition qui le rend un interdit). **Le danger n'est pas le rouge : c'est qu'un banc faux fasse DÉGRADER le produit pour lui plaire.** J'étais parti pour conclure « l'agent ne sait pas enregistrer un service » — en démarrant l'app, la route répondait **200**. Parade livrée : `bench-discoverability.selftest.mjs` (39/39 sondes, deux échantillons figés chacune, `--prove` qui ampute et exige qu'un cas tombe) — **zéro agent, zéro token, quelques secondes**. À graduer en `feedback_*` : le seuil de 3× est dépassé.
- `[1× — 2026-07-28]` **L'auto-contrôle doit appeler l'implémentation, jamais la recopier.** `evaluateProbe` (avec `where`/`invert`/`unless`) est exportée par le banc et importée par son selftest : un contrôle qui réimplémente sa règle ne valide que lui-même, et les trois faux positifs seraient passés à l'identique.
- `[1× — 2026-07-28]` 🔴 **Un obstacle d'ENVIRONNEMENT se lit comme un échec de l'agent.** Tâche 9 déclarée en échec : le transcript montre `command not found: timeout` (**`timeout` n'existe pas sur macOS**) qui a tué ses deux dernières tentatives, plus un `command not found: nodefony` avant qu'il passe à `npx`. L'outil, lui, répondait en 2-3 s avec un JSON parsable. **Avant d'accuser l'agent : lire ce que les commandes lui ont RENDU.**
...
- `[1× — 2026-07-28]` 🔴 **Une doc IA périmée fabrique un faux diagnostic — y compris le mien.** Quatre endroits affirmaient qu'un service s'initialise par `initialize()` ; le hook est `init(owner)` (`Kernel.ts:2784`, `Module.ts:377`), et l'injecteur n'appelle **ni l'un ni l'autre** — c'est l'appelant qui démarre. J'ai corrigé, puis **écrit dans un message de commit** que ces pages avaient égaré l'agent du banc : le transcript dit qu'il n'a ouvert **aucun** fichier du framework ; il a copié le motif du **controller** (`initialize()`, le seul hook asynchrone qu'une app fraîche montre). **La correction était juste, l'attribution inventée** — vérifier le transcript AVANT d'écrire une cause dans un commit.
- `[1× — 2026-07-28]` **Un geste exigé par un gate doit COMPILER.** `nodefony check` réclame `@services([X])` ; sur un service qui redéfinit `get(key)`, l'ajouter ne compile pas — `Service.get<T>(name)` est l'accès au conteneur (`Service.ts:442`), et l'IDE affiche le symptôme sur `@services`, pas la cause (`TS2416`, ligne de la méthode). Surface de collision à connaître : `get set has remove clean log trace on off once emit fire listen`. Un gate dont la voie de sortie casse est un gate qu'on désactive.
...
- `[1× — 2026-07-28c]` 🔴 **Le hook prettier réécrit le markdown APRÈS toute vérification — et il peut CASSER le sens.** Un paragraphe dont une ligne commençait par `+` (continuation de phrase) a été converti en **puce markdown** au commit : le texte s'est coupé en deux, un fragment devenu élément de liste. Vu seulement en relisant le fichier commité. Le contrôle avant `git commit` ne prouve rien sur ce qui EST commité : après un commit qui déclenche un formateur, **relire le rendu final**, pas le fichier qu'on a écrit. (Prettier gagne aussi sur les continuations de `>` en blockquote — cosmétique, mais inutile de lutter.)
...
- `[1× — 2026-07-27i]` 🔴 **Un correctif validé dans le CHECKOUT n'est pas validé dans l'APPLICATION.** J'ai corrigé le scaffold, rebuildé, relancé dans l'app témoin : aucun effet. J'allais conclure que le correctif ne mordait pas. En vrai le shim `bin/nodefony` **délègue au CLI du projet** (règle « le CLI de l'application prime ») — l'app tournait sur sa copie installée, l'ancienne. Pour éprouver une modif du core dans une app : soit une app LIÉE au checkout, soit re-packer. **Le binaire qu'on lance n'est pas forcément le code qu'on vient d'écrire.**
- `[1× — 2026-07-27i]` ⭐ **Un test qui hérite du shell n'éprouve pas ce qu'il croit.** Mon correctif faisait lire l'environnement au générateur ; **4 tests basculaient** si `NF_DATABASE_URL` est posée dans le terminal (docker local = le cas courant). Le développeur aurait vu 4 rouges sans rapport avec son travail. Un test pose son décor **explicitement** — surtout quand le code sous test consulte l'environnement.
- `[1× — 2026-07-27i]` **`npm pack --silent` fait taire npm, pas les scripts qu'il déclenche.** Le `prepack` d'un paquet à frontend déverse tout son build Vite sur la sortie standard : `pack-all.mjs` prenait TOUT pour le nom du tarball → manifeste avec un pavé de 4 ko en guise de nom, et l'install suivante mourait en `ENAMETOOLONG` incompréhensible. **Ne jamais prendre la sortie ENTIÈRE d'une commande pour une valeur** — dernière ligne non vide, et refus si elle n'a pas la forme attendue.
...
- `[1× — 2026-07-28c]` 🔴 **J'ai écrit dans `AGENTS.md` que `timeout` n'existe pas sur macOS — et je suis tombé dedans une heure plus tard.** Mon `timeout 90 npx nodefony inspect … | jq` a rendu VIDE ; j'ai failli conclure « le service n'est pas au conteneur ». Sauvé par le conseil que je venais moi-même d'écrire : relancer **sans le tube**, lire la sortie brute — les trois services y étaient. Documenter un piège ne protège de rien : ce qui protège, c'est de **ne jamais conclure sur une sortie vide sans avoir vu la sortie brute**.
...
- `[2× — 2026-07-27i]` ⭐⭐ **LE DÉCOR DÉCIDE DU VERDICT — et l'inverser a INVERSÉ le résultat.** L'app témoin vivait dans le checkout, paquets symlinkés : haiku lisait les **sources** du framework et n'appelait **jamais** le générateur (0 appel, schéma parfait à la main). Décor refait — app **hors du dépôt**, paquets installés depuis de vraies archives npm, isolation **constatée** avant l'agent : **11 appels au générateur**, dont un `--dry-run` spontané. Même agent, même consigne, même schéma. **On ne convainc pas un agent d'utiliser un outil : on retire le chemin plus court.** Zéro mot ajouté nulle part. Corollaire pour tout banc : un décor plus généreux que la réalité rend un verdict sur une situation qui n'existe chez personne.
- `[1× — 2026-07-27i]` 🔴🔴 **Un rapport qui compte mal ACCUSE LE MAUVAIS COUPABLE.** Le juge cherchait la table `account`, la base contenait `accounts` (le générateur pluralise) → « **0 colonne sur 83** ». Les 83 étaient là. J'ai failli conclure « l'agent n'a rien produit » alors que le défaut était dans l'OUTIL. Remède : chercher le travail **par son contenu** avant de conclure à son absence (≥50 % des colonnes attendues ⇒ c'est la même table, mal nommée) — et le dire dans le rapport, avec la cause.
- `[1× — 2026-07-27i]` 🔴🔴 **Mon nettoyage inter-runs visait des noms que rien ne portait.** `dropExpected` supprimait les tables aux noms ATTENDUS (`account`, `visit`) alors que le générateur crée `accounts`, `visits` : **elles survivaient d'un run à l'autre**, et le juge a relu celles du run PRÉCÉDENT — rendant « 18 uuid dégradés, 32 longueurs perdues » **identiques après le correctif qui devait les supprimer**. J'ai failli conclure que le correctif ne servait à rien. **Un nettoyage se fait sur ce qui EXISTE, jamais sur ce qu'on espère trouver** — et c'est précisément ce que la fonction était censée garantir (« le juge lirait un état, pas un résultat »). Deux gardes tombent ensemble : le nettoyage ET la conclusion.
- `[1× — 2026-07-27i]` 🔴 **L'agent JUGÉ laisse le décor sale — et c'est le juge qui trébuche.** Mon juge HTTP n'a pas pu démarrer : `port déjà en écoute` — l'agent avait laissé son propre serveur debout en fin de tâche. Le piège (« un autre serveur Nodefony occupe les ports ») était **déjà écrit dans le SKILL du banc**, et je l'ai rejoué. Un juge qui démarre quelque chose commence par **arrêter ce qui traîne**, il ne suppose jamais un décor propre.
- `[1× — 2026-07-27i]` 🔴 **Un juge ne doit pas noter ce qu'on n'a pas demandé.** Le juge d'API reprochait à un run ANCIEN une route non protégée — exigence ajoutée après coup. La **consigne s'écrit AVEC le run** (`mission.json`) et se relit à l'analyse ; sinon `--analyze-only` fabrique des échecs rétroactifs.
