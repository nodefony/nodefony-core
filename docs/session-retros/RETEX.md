...
- `[1× — 2026-06-29]` **Restart dev = full turbo rebuild (~2-4 min) + sessions vidées au boot (gc)** → chaque log backend coûte ~4 min ET délogge le testeur (re-repro d'un bug aléatoire repart de zéro). `start.sh` TIMEOUT 25 s = faux négatif (vérif turbo du DevSupervisor) → poller `lsof :5152 -sTCP:LISTEN` + `grep "BOOT ok"`. Motive le chantier debug-runtime (log à chaud sans rebuild).
