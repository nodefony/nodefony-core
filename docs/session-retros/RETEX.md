...
- `[1× — 2026-07-13]` **Une capacité prouvée sur UN chemin n'existe pas forcément sur le chemin voisin** : la dédup de clé d'idempotence marchait au data plane admin (dispositif interne), mais via le pont `api.request` les controllers user n'avaient NI `@Body` (corps ALS non lu) NI la porte `@Idempotent` (méthode logique dans `methodOverride`, `executeAction` appelé nu) → rejeu `socket.mutate` = doublon SILENCIEUX. Attrapé UNIQUEMENT par le banc client réel bout-en-bout (`--link` + RealtimeClient). Un template/saveur qui promet un comportement = un banc qui le prouve côté USER.
