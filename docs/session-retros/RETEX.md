...
- `[1× — 2026-07-03]` **`rg -rn "pattern"` : `-r` = REPLACE (réécrit l'affichage), pas récursif** → output silencieusement corrompu (chaque match du pattern remplacé par la chaîne passée, ex. `watch`→`n`) qui ressemble à du vrai contenu de fichier. Les fichiers ne sont PAS modifiés (affichage only) mais l'audit lit du faux. rg est déjà récursif par défaut : `rg -n` suffit ; vécu 2× dans la même session avant détection.
