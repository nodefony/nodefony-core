...
- `[1× — 2026-06-22]` **`start.sh` timeout = faux négatif possible (turbo retarde le boot > 25 s).** Le serveur a booté quand même (`BOOT ok` dans le log à T+30s). Avant de relancer : `grep "BOOT ok"` le log + `lsof :5152`. Ne pas tuer/relancer un serveur déjà UP sur la foi du seul timeout du script.
