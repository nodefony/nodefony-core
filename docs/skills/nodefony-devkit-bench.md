...
| Famille | Autres |
| Corps | 948 lignes |
| Coût d'activation | ~15 837 tokens (le corps est chargé à l'invocation) |
...
| `scripts/bench-discoverability.selftest.mjs` | Auto-contrôle des sondes du banc de découvrabilité — le juge, AVANT le verdict. | `--analyze-only` `--answers-json` `--describe-json` `--detach` `--dry-run` `--help` `--json` `--kind` `--limit` `--name` `--no` `--no-check` `--no-install` `--prove` `--yes` | `NF_CLE` |
...
| corps < 500 lignes | recommandé | ❌ | 948 | best-practices : corps court (index) + détail en `references/` (divulgation progressive) |
