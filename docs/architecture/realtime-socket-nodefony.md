...
| Un message publié sur un pod n'arrive pas aux clients de l'autre pod | La diffusion entre pods est **opt-in par préfixe**, pas automatique               | Déclarer le préfixe via `RealtimeHub.markBroadcastChannel()` (`RealtimeHub.ts:594`) — cf la page Architecture du module.                           |
