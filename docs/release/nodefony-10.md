...
| **L'application possède son entité `User`** | volet A du kit `project_user_entity_roles_kit` (GO user 2026-07-25). Symfony, Laravel, Django et Rails donnent tous la table `User` à l'application ; ici c'est l'adapter ORM qui la possède, et `create entity User` la refuse depuis 2026-07-25 (sinon le boot casse sur « colonne inconnue ») — donc un utilisateur métier avec des champs en plus n'a **aucune voie de première classe**. **Conception ARRÊTÉE après prototype exécuté et double relecture — voir la mémoire `project_user_entity_roles_kit`, qui fait foi sur le COMMENT.** L'application **possède** la table : `User` est RETIRÉ des `0000_framework_init.sql` du framework (fenêtre pré-publication UNIQUEMENT), l'entité naît dans l'app par `create entity User` — fichier explicite listant les colonnes du contrat, comme `make:user` de Symfony —, et sa migration suit la chaîne ordinaire de l'application. À livrer : le retrait des migrations, le générateur, la garde « champ métier nullable ou avec défaut **SQL** », la vérification AU BOOT (colonne du contrat absente → message franc), le refus franc du fallback en production, le report des champs métier en lecture sur les trois dépôts, et les deux adapters (drizzle + mongoose). **Les FOREIGN KEY SORTENT du périmètre** (ticket propre) : la décision du 2026-07-25 les incluait au motif que « ne pas les émettre crée la divergence dev/prod, la production les posera par ses migrations » — S5 a rendu cette prémisse caduque en faisant dériver les deux chemins de la MÊME source (une entité sans `.references()` ne produit aucune contrainte, nulle part ; vérifié : zéro `REFERENCES` dans les trois migrations livrées). **Pourquoi dans 10.0.0 et pas 10.1** : même raison que le reste de ce tableau — le code d'une application est FIGÉ à sa création, un correctif ultérieur ne réparerait aucune app née en 10.0.0. Le volet B (source de hiérarchie de rôles pluggable) reste en 10.1 : la config statique demeure le défaut, personne n'est cassé. | une app témoin qui POSSÈDE son `User` avec deux champs métier démarre, se connecte, et les écrans Sessions/Users de Studio répondent encore ; une app dont l'entité OUBLIE une colonne du contrat échoue AU BOOT avec le nom de la colonne et de son lecteur ; chaque garde éprouvée par sa preuve négative (débrancher, vérifier que quelque chose tombe) |
...
| 2026-08-27 | **`IClientKernel` RETIRÉ de la surface publiée (révision de l'ADR-0007 D2)** : le contrat du noyau client était gelé SemVer sans qu'aucune implémentation ne l'exerce, et il portait déjà deux défauts qu'un seul `implements` aurait fait tomber (son registre ne pouvait pas nourrir `NodefonyProvider` ; il ne savait pas exprimer le re-handshake d'identité de sa propre décision D9). Coûts asymétriques : publier faux = une **majeure** pour corriger, ajouter une fois exercé = une **mineure**. Le réexport sort du barrel client, la spécification reste au dépôt. Il y revient quand une **application réelle** l'aura éprouvé — **Studio** (#91), et non le debug-client. Gardé par `src/nodefony/src/tests/clientSurfaceExercised.test.ts`, qui refuse toute interface publiée que rien n'exerce | ✅ acté |
