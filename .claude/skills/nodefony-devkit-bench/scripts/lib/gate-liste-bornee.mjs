 ...
 *
 * La valeur `5` n'est pas libre : c'est celle que `garderPortLibre` impose à
 ...
 */
export function compterSemes(corps) {
  return (corps.match(new RegExp(MARQUE_SEMIS, "gu")) ?? []).length;
}
 ...
 */
export function juger({ premier, second, semePremier, semeSecond }) {
 ...
 *
 * @param {Bocal} bocal - cookies (jeton anti-rejeu compris).
 * @param {number} depuis - numéro de départ des références.
 * @param {number} combien - nombre de lignes à poser.
 * @returns {Promise<{poses: number, erreur?: string}>}
 */
async function semer(bocal, depuis, combien) {
  ...
  await demander("GET", ROUTE_CATALOGUE, bocal);
  let poses = 0;
  for (let i = 0; i < combien; i += 1) {
    const n = depuis + i;
    const r = await demander("POST", ROUTE_CATALOGUE, bocal, {
      corps: { reference: `${MARQUE_SEMIS}${n}`, price: 100 + n },
      jeton: bocal.jeton(),
    });
    if (r.erreur) return { poses, erreur: r.erreur };
    if (r.statut === 201 || r.statut === 200) poses += 1;
    else if (poses === 0) {
      return {
        poses,
        erreur: `POST ${ROUTE_CATALOGUE} rend ${r.statut} — ${(r.corps ?? "").slice(0, 160)}`,
      };
    }
  }
  return { poses };
...
async function main() {
  ...
  const v1 = await semer(bocal, 1, VAGUE);
  if (v1.erreur || v1.poses < VAGUE) {
    sortir(
      4,
  ...
  const r1 = await demander("GET", ROUTE_SYNTHESE, bocal);
  if (r1.erreur) {
    sortir(6, `CAUSE=aucune-reponse — GET ${ROUTE_SYNTHESE} : ${r1.erreur}.`);
  }
  if (r1.statut === 404) {
    sortir(
      3,
  ...
  const premier = compterSemes(r1.corps ?? "");

  const v2 = await semer(bocal, VAGUE + 1, VAGUE);
  if (v2.erreur || v2.poses < VAGUE) {
    sortir(
      4,
  ...
  const r2 = await demander("GET", ROUTE_SYNTHESE, bocal);
  if (r2.erreur) {
    sortir(
      6,
      `CAUSE=aucune-reponse — seconde mesure de ${ROUTE_SYNTHESE} : ${r2.erreur}.`,
    );
  }
  const second = compterSemes(r2.corps ?? "");

  const { code, message } = juger({
    premier,
    second,
    semePremier: VAGUE,
    semeSecond: VAGUE * 2,
