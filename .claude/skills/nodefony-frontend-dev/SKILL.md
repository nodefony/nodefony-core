...
- **socket Nodefony** : `RealtimeClient` (subscribe/request/mutate/ping), socle agnostique `observe*` (une liaison de vue ne contient QUE rappel+libération → réactivité), liaisons idiomatiques `nodefony/react` (hooks), `nodefony/vue` (plugin + composables) et `nodefony/angular` (fournisseur + fonctions d'injection, ZÉRO décorateur publié), canaux temps réel.
...
| Socket client `RealtimeClient`, socle agnostique `observe*`/`connectShared` (Svelte) + hooks `nodefony/react`, composables `nodefony/vue`, injection `nodefony/angular` | `references/realtime-client.md`                 |
