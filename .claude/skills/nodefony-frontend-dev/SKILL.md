...
| **Voir l'écran soi-même** — navigateur en conteneur (console, a11y, requêtes) + ses 3 contraintes | `references/build-hmr.md` §9                    |
...
3. **Voir l'écran** — deux voies, dans cet ordre :
   - **Navigateur en CONTENEUR** (`docker compose -f docker/docker-compose.yml --profile browser up -d`) :
     lit la console, l'arbre d'accessibilité et les requêtes réelles, sans rien installer sur le poste.
     C'est l'« environnement isolé » que la règle anti-headless exigeait — Chromium n'y tourne pas
     à nu sur la machine, il est borné (2 CPU / 2 Go) et meurt au `down`. **Trois contraintes, toutes
     structurelles** → `references/build-hmr.md` §9 : joindre l'hôte par `host.docker.internal` (jamais
     `localhost`, qui désigne le conteneur) ; passer par **HTTPS 5152** car le cookie de session est
     `secure` et serait jeté sur une origine `http://` non-`localhost` ; et servir l'UI en
     `NF_STUDIO_UI=static`, **le HMR n'étant pas joignable depuis un conteneur** (P14.17).
