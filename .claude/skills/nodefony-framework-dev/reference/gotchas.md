...
- **typecheck = gate DISTINCT du build** : rollup vert peut masquer un `tsc --noEmit` rouge (ex. TS18036 sur `static #` + décorateur de classe → `private static`). Hook pre-push.
...
- **Citer la RFC EXACTE** (skill `nodefony-rfc` / `reference/rfc/`) plutôt que trancher « raisonnable de mémoire ».
