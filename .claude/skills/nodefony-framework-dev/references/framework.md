...
- contrat partagé = types `I*Api`/`I*Controller` exportés (jamais une copie figée). Voir `nodefony-studio-dev` → `references/realtime.md` + recette « page données ».
...
> PROPRES au framework**. NE duplique PAS : recettes d'usage → `references/recipes-{core,http,admin}.md` ;
> gotchas transverses (idempotence codes RFC, hot-path, ALS, redirect whitelist…) → `references/gotchas.md` ;
> sécurité (coder en sûreté, sources) → `references/security.md`.
>
...
> Recette PRODUCTEUR (créer un endpoint admin) = `references/framework.md`. Ici = surface + internals.
