...
| `db-readiness-pod.mjs`   | ce que la sonde de disponibilité fait d'un schéma qui ne colle pas : base en AVANCE (mise à jour progressive, retour arrière) → le pod sert ; empreinte réécrite → il ne sert pas, mais reste vivant | `node db-readiness-pod.mjs`                 |
| `db-persistance-pod.mjs` | la chaîne SYSTÈME de bout en bout : une donnée écrite par une requête HTTP survit-elle à l'arrêt gracieux ET au redémarrage du pod — empreinte comparée, transaction en échec sans résidu            | `node db-persistance-pod.mjs`               |
